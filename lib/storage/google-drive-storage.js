...
async function getAccessToken() {
  ...
  if (!response.ok || !tokenData.access_token) {
    const tokenError = tokenData.error_description || tokenData.error || response.statusText;
    if (/expired|revoked|invalid_grant/i.test(tokenError)) {
      throw new Error("Google Drive authorization expired or was revoked. Generate a new GOOGLE_DRIVE_REFRESH_TOKEN and restart the server.");
    }
    throw new Error(`Google Drive token refresh failed: ${tokenError}`);
  }
