...
function buildKeywordSignature(text) {
  const keywords = [
    "Policy No", "Policy Number", "Insured", "Proposer", "Registration No", "Engine No", "Chassis No",
    "Total Premium", "Gross Premium", "GST", "Total IDV", "Nominee", "Hypothecation", "Previous Policy",
    "Risk Location", "Premises to be Insured", "MSME Suraksha"
  ];
...
function findRepeatedLines(text) {
  ...
  String(text || "").split(/\n/).map((line) => line.trim()).filter((line) => line.length >= 18).forEach((line) => {
    counts.set(line, (counts.get(line) || 0) + 1);
  });
  return Array.from(counts.entries()).filter(([, count]) => count > 1).slice(0, 12).map(([line, count]) => ({ line, count }));
}
