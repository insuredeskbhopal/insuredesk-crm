function extractWithSchema(text = "", understanding = {}, schema = {}) {
  ...
  for (const field of fields) {
    ...
    if (field.required && result.value == null) {
      ...
      warnings.push({ field: field.field, warning: "Low confidence extraction", confidence: result.confidence });
    }
  }

  const populated = Object.values(extractedFields).filter((field) => field.value != null).length;
  const highConfidence = Object.values(extractedFields).filter((field) => field.value != null && field.confidence >= 0.72).length;
  const confidence = Number(Math.min(0.99, ((schema.schemaMatch || 0) * 0.45) + ratio(populated, fields.length) * 0.25 + ratio(highConfidence, fields.length) * 0.3).toFixed(2));
...
function extractField(text, understanding, schemaField, mapEntry) {
  ...
  const haystacks = [
    { text: sectionText, sectionBoost: sectionTypeBoost(sectionType), page: mapEntry?.sectionMatch?.page || 1, scope: "section" },
...
function normalizeTypedValue(value, type) {
  ...
  if (type === "registration") {
    return text.match(/(?:[A-Z]{2}[-\s]?\d{1,2}[-\s]?[A-Z]{1,3}[-\s]?\d{4}|[A-Z]{2}[-\s]\d{1,2}[-\s]\d{4})/i)?.[0] || null;
  }
  if (type === "percent") {
  ...
  return text
    .split(/\s{3,}|(?=\b(?:Policy|Proposal|Invoice|Customer|From|To|Make|Model|RTO|GSTIN|Premium|Document generated|UIN)\b)/i)[0]
    .replace(/^is\s+/i, "")
    .trim() || null;
}
...
function sectionTypeBoost(sectionType) {
  if (["policy_details", "insured_details", "vehicle_details", "premium_details"].includes(sectionType)) return 0.14;
  if (["gst_table", "previous_policy_details", "nominee_details", "financier_details"].includes(sectionType)) return 0.1;
  return 0.06;
}
