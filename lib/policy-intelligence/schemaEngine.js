...
const { buildFieldMap } = require("../pdf-understanding/buildFieldMap");

const SCHEMA_DIR = path.join(process.cwd(), "lib", "policy-schemas");
...
function readSchema(fileName) {
  ...
  try {
    ...
    return { schema: null, error: `Schema file invalid: ${fileName}: ${error instanceof Error ? error.message : "Unknown error"}` };
  }
}
...
function calculateSchemaMatch(understanding, schema) {
  ...
  const fieldAliasHits = fields.filter((field) => (field.aliases || []).some((alias) => labels.has(normalize(alias)))).length;
  const requiredHits = fields.filter((field) => field.required && (sectionTypes.has(field.section) || (field.aliases || []).some((alias) => labels.has(normalize(alias))))).length;
  const expectedSectionHits = (schema.expectedSections || []).filter((section) => sectionTypes.has(section)).length;
