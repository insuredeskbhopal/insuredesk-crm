...
export function resolveInsurerTemplateId(company = "") {
  ...
  return INSURER_TEMPLATES.find((template) => template.matches.some((item) => normalized.includes(item)))?.id || "";
}
...
export function buildEndorsementScheduleData(form = {}) {
  ...
  const data = {
    ...
    policyTitle: form.policyType || raw.productName || raw.policyType || "MSME Suraksha Kavach Package Policy - Advance",
    insuredName: form.insuredName || raw.insuredName || raw.customerName || "",
    mailingAddress: form.mailingAddress || raw.mailingAddress || raw.communicationAddress || form.address || "",
  ...
  data.endorsementWording = form.endorsementWording || form.description || form.newValues?.reason || generateEndorsementWording(data);
  return data;
}
...
function premiumSentence(data, config = {}) {
  ...
  const isRefund = config.premiumMode === "refund" || /^-/.test(text.trim()) || /refund|refundable/i.test(text);
...
function buildChangeSummary(data, subject, action) {
  ...
  const changes = keys.map((key) => changeSentence(key, oldValues[key], newValues[key], subject, action)).filter(Boolean);
...
function normalizePremiumAmount(value) {
  ...
  if (!text) return "";
  const cleaned = text.replace(/refund|refundable|rs\.?|inr|\/-|,/gi, "").replace(/[^\d.-]/g, "").trim();
