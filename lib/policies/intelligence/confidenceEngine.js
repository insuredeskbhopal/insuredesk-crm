function validateExtraction({ legacyData = {}, schemaResult = {}, understanding = {} }) {
  ...
  if ((understanding.confidence || 0) < 0.55) {
    warnings.push({
      field: "document",
      warning: "Low document understanding confidence",
  ...
  if ((schemaResult.schemaMatch || 0) < 0.62) {
    warnings.push({
      field: "schema",
      warning: "Low schema match; regex fallback used",
      confidence: schemaResult.schemaMatch || 0,
    });
  }
  for (const [key, result] of Object.entries(fieldConfidence)) {
    if (result?.value != null && result.confidence < 0.55) {
      warnings.push({
        field: key,
        warning: "Very low confidence field candidate",
