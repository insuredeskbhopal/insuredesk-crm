export function normalizePolicyFamily(record = {}) {
  ...
  if (/\b(fire|sfsp|standard\s+fire|msme\s+suraksha|burglary|warehouse|stock|contents|property|industrial\s+all\s+risk)\b/.test(haystack)) {
    return "Fire Policy";
  }
  if (/\b(health|mediclaim|medical|family\s+floater|critical\s+illness|hospital|personal\s+accident|pa policy)\b/.test(haystack)) {
