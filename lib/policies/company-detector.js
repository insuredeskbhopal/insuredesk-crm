...
function isCompanyEvidenceLine(line = "") {
  ...
  return (
    /\b(?:insurance|assurance|insurer|company|co\.?|ltd\.?|limited|general\s+insurance)\b/i.test(text) &&
    /[a-z]/i.test(text) &&
    text.length >= 5 &&
    text.length <= 180
  );
...
function getCompanyTokens(value = "") {
  return String(value || "")
    ...
    .filter(
      (token) =>
        !["the", "and", "company", "limited", "ltd", "insurance", "assurance", "general"].includes(token),
    );
}
