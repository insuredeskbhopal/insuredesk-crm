...
function extractNewIndiaMotorDetails(text = "") {
  ...
  result.financerName = cleanHdfcValue(matchGroup(vehicleBlock, /Name\s+of\s+the\s+Financier\s*:?\s*([A-Z0-9 .&/-]+?)(?=\s+Chassis|\s+Cover\s+Note|\n|$)/i));
...
function extractNewIndiaEnhancedCovers(text = "") {
  ...
  const hasYes = (label) => new RegExp(`${label}[\\s\\S]{0,30}?Yes`, "i").test(block);
  ...
  return {
    ...
    roadsideAssistance: hasYes("Roadside Assistance"),
