...
function extractTataAigMakeModelVariant(text = "") {
  const raw =
    cleanHdfcValue(matchGroup(text, /Make\s*\/\s*Model\s*\/[\s\S]*?Variant\s*\n?\s*([^\n]+)/i)) ||
    extractTataAfterLabel(
      text,
      ["Make / Model / Variant", "Make / Model /"],
      ["Fuel Type", "Engine Number", "Engine No.", "Chassis No.", "Body Type", "CC/KW", "CC / KW"]
    ).replace(/^Variant\s+/i, "");
