...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  if (iffcoWarehouse.documentDetected) {
    const legacyData = {
      ...
      groupName: deriveGroupName(sourceText, sourceFile, iffcoWarehouse.insuredName, "MPWLC"),
      ...
      occupancy: iffcoWarehouse.businessDescription,
...
function extractIffcoWarehouse(text, filename = "") {
  ...
  if (isFidelity) {
    ...
    sumInsured = normalizeIffcoWarehouseAmount(matchGroup(text, /Total Sum Insured\s*([\d,.]+)/i));
    if (isFire) {
