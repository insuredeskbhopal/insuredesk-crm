...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  if (iciciWarehouse.documentDetected) {
    const legacyData = {
      ...
      sumInsured: iciciWarehouse.contentsSumInsured,
  ...
  const legacyData = {
    ...
    sourceText,
    status: "saved",
    documentFormat: hdfcErgoMotor.documentDetected ? "HDFC_ERGO_MOTOR_V1" : generaliMotor.documentDetected ? "GENERALI_MOTOR_V1" : newIndiaMotor.documentDetected ? "NEW_INDIA_MOTOR_V1" : iffcoMotor.documentDetected ? "IFFCO_TOKIO_MOTOR_V1" : understoodDocumentFormat !== "GENERIC_POLICY_V1" ? understoodDocumentFormat : "",
    documentCategory: hdfcErgoMotor.documentDetected || generaliMotor.documentDetected || newIndiaMotor.documentDetected || iffcoMotor.documentDetected ? "Motor Insurance" : policyUnderstanding.documentCategory || "",
    insuredName: hdfcErgoMotor.insuredName || generaliMotor.insuredName || newIndiaMotor.insuredName || iffcoMotor.insuredName || insuredName,
    ...
    groupName,
    policyNumber: hdfcErgoMotor.policyNumber || generaliMotor.policyNumber || newIndiaMotor.policyNumber || iffcoMotor.policyNumber || policyNumber,
    policyType: newIndiaMotor.policyType || generaliMotor.policyType || iffcoMotor.policyType || policyType,
    sumInsured: hdfcErgoMotor.totalIdv || generaliMotor.totalIdv || newIndiaMotor.totalIdv || iffcoMotor.totalIdv || motorVehicleTable.idv || sumInsured,
    premium: hdfcErgoMotor.totalPremium || generaliMotor.totalPremium || iffcoMotor.totalPremium || newIndiaPremiumSchedule.totalPremium || genericPremiumSchedule.totalPremium || premium,
    totalPremium: hdfcErgoMotor.totalPremium || generaliMotor.totalPremium || iffcoMotor.totalPremium || newIndiaPremiumSchedule.totalPremium || genericPremiumSchedule.totalPremium,
    netPremium,
    tpDriverOwner,
    odPremium,
    startDate: hdfcErgoMotor.policyStartDate || generaliMotor.policyStartDate || newIndiaMotor.policyStartDate || iffcoMotor.policyStartDate || startDate,
    expiryDate: hdfcErgoMotor.policyEndDate || generaliMotor.policyEndDate || newIndiaMotor.policyEndDate || iffcoMotor.policyEndDate || expiryDate,
    duration: buildDuration(hdfcErgoMotor.policyStartDate || generaliMotor.policyStartDate || newIndiaMotor.policyStartDate || iffcoMotor.policyStartDate || startDate, hdfcErgoMotor.policyEndDate || generaliMotor.policyEndDate || newIndiaMotor.policyEndDate || iffcoMotor.policyEndDate || expiryDate) || duration,
    riskLocation: hdfcErgoMotor.communicationAddress || generaliMotor.communicationAddress || newIndiaMotor.insuredAddress || riskLocation,
    district,
    tehsil,
    insuranceCompany: hdfcErgoMotor.companyName || generaliMotor.companyName || newIndiaMotor.companyName || iffcoMotor.companyName || normalizeInsuranceCompanyName(insuranceCompany, sourceText),
    description: businessDescription,
    pptMpwlc,
    occupancy,
    validIn,
    vehicleNumber: hdfcErgoMotor.registrationNumber || generaliMotor.registrationNumber || newIndiaMotor.registrationNumber || iffcoMotor.registrationNumber || vehicleNumber,
    registrationNumber: hdfcErgoMotor.registrationNumber || generaliMotor.registrationNumber || newIndiaMotor.registrationNumber || iffcoMotor.registrationNumber || registrationNumber,
    makeModel: [hdfcErgoMotor.vehicleMake, hdfcErgoMotor.vehicleModel].filter(Boolean).join(" ") || [generaliMotor.vehicleMake, generaliMotor.vehicleModel].filter(Boolean).join(" ") || newIndiaMotor.makeModel || iffcoMotor.makeModel || makeModel,
    variant: hdfcErgoMotor.variant || generaliMotor.variant || newIndiaMotor.variant || iffcoMotor.variant || variant,
    manufacturingYear: hdfcErgoMotor.manufacturingYear || generaliMotor.manufacturingYear || newIndiaMotor.manufacturingYear || iffcoMotor.manufacturingYear || manufacturingYear,
    registrationDate,
    engineNumber: hdfcErgoMotor.engineNumber || generaliMotor.engineNumber || newIndiaMotor.engineNumber || iffcoMotor.engineNumber || engineNumber,
    chassisNumber: hdfcErgoMotor.chassisNumber || generaliMotor.chassisNumber || newIndiaMotor.chassisNumber || iffcoMotor.chassisNumber || chassisNumber,
    fuelType: generaliMotor.documentDetected ? generaliMotor.fuelType || "" : newIndiaMotor.fuelType || fuelType,
    cubicCapacity: hdfcErgoMotor.cubicCapacity || generaliMotor.cubicCapacity || newIndiaMotor.cubicCapacity || iffcoMotor.cubicCapacity || cubicCapacity,
    seatingCapacity: hdfcErgoMotor.seatingCapacity || generaliMotor.seatingCapacity || newIndiaMotor.seatingCapacity || iffcoMotor.seatingCapacity || seatingCapacity,
    grossVehicleWeight: newIndiaMotor.grossVehicleWeight || iffcoMotor.grossVehicleWeight || grossVehicleWeight,
    idv: hdfcErgoMotor.totalIdv || generaliMotor.totalIdv || newIndiaMotor.totalIdv || iffcoMotor.totalIdv || motorVehicleTable.idv || idv,
    ncb: hdfcErgoMotor.ncbPercentage || generaliMotor.ncbPercentage || iffcoMotor.ncbPercentage || newIndiaPremiumSchedule.ncbPercentage || ncb,
    policyCoverType,
    ...
    financerName: newIndiaMotor.documentDetected ? newIndiaMotor.financerName : (generaliMotor.financerName || iffcoMotor.financerName || financerName),
    companyName: hdfcErgoMotor.companyName || generaliMotor.companyName || newIndiaMotor.companyName || iffcoMotor.companyName,
    ...
    communicationAddress: hdfcErgoMotor.communicationAddress || generaliMotor.communicationAddress || newIndiaMotor.insuredAddress,
    customerMobile: hdfcErgoMotor.customerMobile || generaliMotor.customerMobile || newIndiaMotor.customerMobile,
    ...
    vehicleMake: hdfcErgoMotor.vehicleMake || generaliMotor.vehicleMake || newIndiaMotor.vehicleMake || iffcoMotor.vehicleMake || genericVehicleParts.make,
    vehicleModel: hdfcErgoMotor.vehicleModel || generaliMotor.vehicleModel || newIndiaMotor.vehicleModel || iffcoMotor.vehicleModel || genericVehicleParts.model,
    ...
    totalIdv: hdfcErgoMotor.totalIdv || generaliMotor.totalIdv || newIndiaMotor.totalIdv || iffcoMotor.totalIdv || motorVehicleTable.idv || idv,
    ...
    gstAmount: hdfcErgoMotor.gstAmount || generaliMotor.gstAmount || iffcoMotor.gstAmount || newIndiaPremiumSchedule.gstAmount || genericPremiumSchedule.gstAmount,
    ...
    previousPolicyNumber: hdfcErgoMotor.previousPolicyNumber || generaliMotor.previousPolicyNumber || iffcoMotor.previousPolicyNumber,
    ...
    ncbPercentage: hdfcErgoMotor.ncbPercentage || generaliMotor.ncbPercentage || iffcoMotor.ncbPercentage || newIndiaPremiumSchedule.ncbPercentage || ncb,
...
function isNewIndiaFinalData(data = {}) {
  return isNewIndiaAssuranceText(data.sourceText || "") ||
...
function finalizeNewIndiaMotorFields(data = {}) {
  ...
  if (premiumSchedule.basicThirdPartyLiability) data.basicThirdPartyLiability = premiumSchedule.basicThirdPartyLiability;
  if (premiumSchedule.zeroDepreciationCover) data.zeroDepreciationCover = premiumSchedule.zeroDepreciationCover;
  if (premiumSchedule.ncbPercentage) {
    data.ncb = premiumSchedule.ncbPercentage;
    data.ncbPercentage = premiumSchedule.ncbPercentage;
  }

  if (/Standalone\s+Motor\s+Own\s+Damage|Stand[-\s]*alone\s+Own\s+Damage|Own\s+Damage\s+Policy/i.test(data.policyType || sourceText)) {
    data.policyCoverType = "Own Damage";
  }
}

function isIffcoFinalData(data = {}) {
  return isIffcoTokioMotorText(data.sourceText || "") ||
...
function isTataAigFinalData(data = {}) {
  return isTataAigMotor(data.sourceText || "") ||
...
function protectTataAigMergedFields(mergedData, legacyData) {
  ...
  const sourceText = mergedData.sourceText || legacyData.sourceText || "";
  const totalPremium = extractTataAigTotalPremium(sourceText) || extractTataAigTotalPremium(legacyData.sourceText || "");
  if (totalPremium) {
  ...
  const netPremium = normalizeAmount(matchGroup(sourceText, /Net Premium\s*\(A\+B\+C(?:\+D)?\)\s*₹?\s*([0-9,.]+)/i));
...
function extractIciciWarehouseMsme(text) {
  ...
  const required = {
    ...
    policyNumber: matchGroup(text, /Policy No\s*([0-9]{4}\/[0-9]+\/[0-9]{2}\/[0-9]{3})/i) ||
      matchGroup(text, /PolicyNo\s*([0-9]{4}\/[0-9]+\/[0-9]{2}\/[0-9]{3})/i),
    insuredName: cleanHdfcValue(matchGroup(text, /Name of the Insured\s*([A-Z0-9 .&/()-]+?)\s*Policy No/i)),
    mailingAddress,
    riskLocation,
    startDate: period.startDate,
    expiryDate: period.expiryDate,
    issuedAt: cleanHdfcValue(matchGroup(text, /Issued at\s*([A-Z ]+?)\s*Premises to be Insured/i)),
    businessDescription,
...
function isIciciWarehouseMsme(text) {
  return /ICICI\s+Lombard\s+General\s+Insurance/i.test(text) &&
    /MSME\s+Suraksha\s+Kavach\s+Package\s+Policy\s*-\s*Advance/i.test(text) &&
    /Premises\s+to\s+be\s+Insured/i.test(text) &&
    /Section\s+wise\s+details/i.test(text);
}

function extractIciciWarehouseCoverages(text) {
  const coveragePatterns = [
    ["MSME Suraksha Kavach - Contents", /MSME\s+Suraksha\s+Kavach\s*-\s*Contents\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i],
    ["Burglary", /(?:\n|\s)2\s*Burglary\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i],
    ["Fidelity", /(?:\n|\s)3\s*Fidelity\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i]
  ];
  return coveragePatterns
    .map(([sectionName, pattern]) => ({ sectionName, sumInsured: normalizeAmount(matchGroup(text, pattern)) }))
    .filter((item) => item.sumInsured);
}

function extractIciciWarehousePremium(text) {
  return {
    premiumIncludingGst: normalizeAmount(matchGroup(text, /Premium\s*\(`\)\s*\(Including GST\)\(`\)\s*([0-9,]+(?:\.\d{2})?)/i)) ||
      normalizeAmount(matchGroup(text, /Total Premium inclusive Tax\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i)),
    netPremium: normalizeAmount(matchGroup(text, /Total value of services\s*\(Premium Value without Tax\)\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i)),
    gstAmount: normalizeAmount(matchGroup(text, /Total Tax Amount\s*\(`\)\s*([0-9,]+(?:\.\d{2})?)/i)) ||
      normalizeAmount(matchGroup(text, /Total Tax Amount\s*\(`\)\s*([0-9]+)/i)),
    cgst: normalizeAmount(matchGroup(text, /CGST\s*9\s*([0-9,]+(?:\.\d{2})?)/i)),
    sgst: normalizeAmount(matchGroup(text, /SGST\s*9\s*([0-9,]+(?:\.\d{2})?)/i)),
    igst: normalizeAmount(matchGroup(text, /IGST\s*0\s*([0-9,]+(?:\.\d{2})?)/i)) || "",
    invoiceNumber: matchGroup(text, /Invoice Number\s*([0-9]+)/i),
    invoiceDate: matchGroup(text, /Invoice Date\s*(\d{1,2}\/\d{1,2}\/\d{4})/i),
    gstin: matchGroup(text, /GSTIN\s*([0-9A-Z]{15})/i) || matchGroup(text, /Invoice Date\d{1,2}\/\d{1,2}\/\d{4}GSTIN([0-9A-Z]{15})/i),
    placeOfSupply: cleanHdfcValue(matchGroup(text, /Place of Supply\s*:\s*([A-Z ]+)/i))
  };
}

function extractIciciWarehouseHypothecation(text) {
  const bankName = cleanHdfcValue(matchGroup(text, /Annexure\s*-\s*I[\s\S]+?Sr\. No\.Name of the BankType of charge\s*1\s*([A-Z0-9 .&/-]+?)\s*Hypothecation/i)) ||
    (/\bMPWLC\b/i.test(text) ? "MPWLC" : "");
  const chargeType = bankName ? "Hypothecation" : "";
  return { bankName, chargeType };
}

function extractIciciWarehouseBroker(text) {
  const compact = text.replace(/\s+/g, " ");
  const match = compact.match(/Agency\/Broker CodeAgency\/Broker NameAgency\/Broker Mobile NoAgency\/Broker Email-ID\s*(\d{10,})\s*([A-Z0-9 .&/-]+?)\s*([6-9]\d{9})\s*([A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,})/i);
  return {
    brokerCode: match?.[1] || "",
    brokerName: cleanHdfcValue(match?.[2] || ""),
    brokerMobile: match?.[3] || "",
    brokerEmail: match?.[4] || ""
  };
}
...
function extractIciciWarehouseSpecialConditions(text) {
  ...
  if (/hazardous goods/i.test(text)) {
    conditions.push("Storage warranty excludes hazardous goods of Category I, II, III, coir waste, coir fibre and caddies.");
  }
...
function cleanWarehouseAddress(value) {
  return cleanHdfcValue(String(value || "")
    .replace(/--\./g, ", ")
    .replace(/--/g, ", ")
    .replace(/\s+\.\s*/g, ", ")
    .replace(/\s*,\s*,/g, ",")
...
function extractIffcoMotorDetails(text = "", vehicleTable = {}) {
  ...
  const tableEngine = isValidIffcoEngine(vehicleTable.engineNumber) && vehicleTable.engineNumber !== tableChassis ? vehicleTable.engineNumber : "";
...
function extractIffcoPolicyPeriod(text = "") {
  ...
  const regular = text.match(new RegExp(`Period\\s+of\\s+Insurance\\s+From\\s*:?\\s*${date}(?:\\s+\\d{2}:\\d{2}:\\d{2})?[\\s\\S]{0,120}?To\\s*:?\\s*Midnight\\s+On\\s+${date}`, "i"));
  if (regular) return { startDate: regular[1], expiryDate: regular[2] };

  const old = text.match(new RegExp(`Policy\\s+effective\\s+from\\s+\\d{3,4}\\s+hrs\\s+${date}[\\s\\S]{0,120}?To\\s+MidNight\\s+${date}`, "i"));
...
function extractIffcoPremiumSchedule(text = "") {
  ...
  const result = {
    ...
    tpPremium: standaloneOd ? "" : normalizeAmount(matchGroup(text, new RegExp(`Net\\s*\\(B\\)\\s*₹?\\s*${money}`, "i"))),
    netPremium: "",
    gstAmount: "",
    cgst: "",
    sgst: "",
  ...
  result.netPremium = normalizeAmount(
    matchGroup(text, /Section\s+1\s*(?:\(A\s*\+\s*B\))?\s*(?:\(for\s*\d+\s*years?\))?\s*Rs\.?\s*([\d,]+(?:\.\d{1,2})?)/i) ||
  ...
  if (!result.netPremium && result.odPremium && result.tpPremium) result.netPremium = sumAmounts(result.odPremium, result.tpPremium);
...
function extractIffcoChassisNumber(text = "") {
  const block = extractMotorVehicleBlock(text) || text;
  const labeled =
    matchGroup(block, /Chassis\s+No\.?[\s\S]{0,140}?([A-Z0-9]{10,25})/i);
...
function isValidIffcoEngine(value = "") {
  ...
  return /^[A-Z0-9]{6,25}$/.test(cleaned) &&
...
function isValidIffcoChassis(value = "") {
  const cleaned = String(value || "").replace(/[^A-Z0-9]/gi, "").toUpperCase();
  return /^[A-Z0-9]{10,25}$/.test(cleaned) &&
    !/^MP\d{2}/i.test(cleaned) &&
    !/^(SEATING|CHASSIS|ENGINE|MAKE|CAPACITY|PACKAGE|COMPREHENSIVE)$/i.test(cleaned);
}
...
function extractIffcoFinancer(text = "") {
  const value =
    matchGroup(text, /Under\s+Hire\s+Purchase\s*\/\s*Hypothecated\s*\/\s*Lease\s+Agreement\s+with\s+(.+?)(?:\s+Nominee|\s+Nominees|\n|Subject)/i) ||
    matchGroup(text, /Under\s+Hire\s+Purchase\/Hypo\/\s*Lease\s+Agreement\s+with\s+(.+?)(?:\n|Subject)/i);
  return cleanHdfcValue(value);
}

function extractIffcoPreviousPolicy(text = "") {
  const match = text.match(/Previous\s+Policy\s+(?:Number|No\.)\s+Previous\s+Insurer\s+Name\s+and\s+Address\s+(?:Policy|Previous)\s+Expiry\s+Date\s+([A-Z0-9/-]+)\s+(.+?)\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
  return {
...
function extractNewIndiaMotorDetails(text = "") {
  ...
  result.policyNumber = matchGroup(text, /Policy\s+Number\s*:?\s*(\d{10,})/i) || matchGroup(text, /Policy\s+No\.?\s*:?\s*(\d{10,})/i);
  ...
  result.insuredAddress = cleanHdfcValue(matchGroup(text, /Insured'?s?\s+Address\s+([\s\S]+?)\s+Contact\s+Number/i));
  ...
  result.previousInsurer = cleanHdfcValue(matchGroup(text, /Previous\s+Insurer\s+(.+?)\s+Previous\s+Policy\s+Number/i));
  result.previousPolicyNumber = cleanHdfcValue(matchGroup(text, /Previous\s+Policy\s+Number\s+([A-Z0-9/.-]+)/i));

  const vehicleBlock = sliceText(text, /VEHICLE DETAILS/i, /INSURED DECLARED VALUE|ENHANCED COVER|SCHEDULE OF PREMIUM/i) || extractMotorVehicleBlock(text) || text;
  result.registrationNumber =
    normalizeRegistrationDisplay(matchGroup(vehicleBlock, /Registration\s+Number\s*([A-Z]{2}[-\s]?\d{1,2}[-\s]?[A-Z]{1,3}[-\s]?\d{3,4})/i)) ||
    normalizeRegistrationDisplay(matchGroup(vehicleBlock, /Registration\s+no\.?\s*([A-Z]{2}[-\s]?\d{1,2}[-\s]?[A-Z]{0,3}[-\s]?\d{0,4})/i)) ||
    normalizeRegistrationDisplay(matchGroup(vehicleBlock, /Registration\s+Number\s*(New Vehicle)/i)) ||
    "";

  const ce = matchGroup(vehicleBlock, /Chassis\s+no\.?\s*\/\s*Engine\s+(?:Number|no\.?)\s*:?\s*([A-Z0-9\s-]+\/[A-Z0-9\s-]+)/i);
  if (ce) {
  ...
  result.makeModel = cleanHdfcValue(matchGroup(vehicleBlock, /Make\s*\/?\s*Model\s*:?\s*([A-Z0-9 &/().,-]+?)(?=Registration\s+no\.?|\s+Registration\s+no\.?|\s+Variant|\s+Year|\n|$)/i));
  ...
  result.variant = cleanHdfcValue(matchGroup(vehicleBlock, /Variant\s*:?\s*([\s\S]{1,80}?)(?=\s+Automobile|\s+Colour|\s+Cover\s+Note|\n|$)/i));
  result.manufacturingYear = matchGroup(vehicleBlock, /Year\s+of\s+manufacture\s*:?\s*(19\d{2}|20\d{2})/i);

  const bodyFuel = cleanHdfcValue(matchGroup(vehicleBlock, /Type\s+of\s+body\s*\/\s*Type\s+of\s+Fuel\s*:?\s*([A-Z -]+\/[A-Z]+)/i));
  if (bodyFuel.includes("/")) {
    ...
    result.fuelType = normalizeNewIndiaFuelType(matchGroup(vehicleBlock, /Type\s+of\s+fuel\s*:?\s*([A-Z]+)/i));
  }

  result.cubicCapacity = matchGroup(vehicleBlock, /Cubic\s+capacity\s*\(cc\)\s*\/\s*Wattage\s*\(kW\)\s*:?\s*(\d+)\s*cc/i) || matchGroup(vehicleBlock, /Cubic\s+capacity[\s\S]{0,50}?(\d+)\s*cc/i);
  result.seatingCapacity = matchGroup(vehicleBlock, /Seating\s+capacity\s+including\s+Driver\s*:?\s*(\d+)/i);
  result.grossVehicleWeight = String(matchGroup(vehicleBlock, /Gross\s+Vehicle\s+Weight\s*\(GVW\)\s*:?\s*([0-9,]+)/i) || "").replace(/[,\s]/g, "");
  result.commercialVehicleType = cleanHdfcValue(matchGroup(vehicleBlock, /Type\s+of\s+Commercial\s+Vehicles\s*:?\s*([\s\S]+?)\s+Sub\s+Type/i));
  result.commercialVehicleSubType = cleanHdfcValue(matchGroup(vehicleBlock, /Sub\s+Type\s*:?\s*([\s\S]+?)\s+Name\s+of\s+the\s+Financier/i));
  result.rtoLocation = cleanHdfcValue(matchGroup(vehicleBlock, /Name\s+of\s+registration\s+authority\s*:?\s*([A-Z ]+?)(?=\s+FASTag|\s+INSURED|\n|$)/i));
  result.financerName = cleanHdfcValue(matchGroup(vehicleBlock, /Name\s+of\s+the\s+Financier\s*:?\s*([\s\S]+?)(?=\s+Chassis|\s+Cover\s+Note|\n\s*Cover\s+Note|$)/i));
  ...
  const idvRow = idvBlock.match(/Vehicle\s+Trailer[\s\S]{0,160}?\n\s*([0-9,]+)\s+[0-9,]+\s+(?:N\/A|0)\s+(?:N\/A|0)\s+[0-9,]*\s+([0-9,]+)/i);
  result.totalIdv = normalizeAmount(idvRow?.[2]) || normalizeAmount(matchGroup(text, /For individual covers \(OD\) in RS\s*:?\s*([0-9,]+)/i)) || extractIDV(text);
...
function extractNewIndiaPolicyPeriod(text = "") {
  const bundled = text.match(/Period\s+of\s+cover\s+OD\s+Cover\s+(\d{1,2}\/\d{1,2}\/\d{4})[\s\S]{0,80}?to\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
  if (bundled) return { startDate: bundled[1], endDate: bundled[2] };

  const regular = text.match(/Period\s+of\s+cover\s+(\d{1,2}\/\d{1,2}\/\d{4})[\s\S]{0,80}?to\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
...
function extractNewIndiaEnhancedCovers(text = "") {
  const block = sliceText(text, /ENHANCED COVER|Cover Description\s+Cover Opted/i, /SCHEDULE OF PREMIUM/i) || "";
  const hasYes = (label) => {
...
function cleanVehicleCode(value = "") {
  return String(value || "")
    .replace(/(?:MAKE|TYPE\s*OF\s*FUEL|TYPE\s*OF\s*BODY|CUBIC\s*CAPACITY|SEATING\s*CAPACITY|REGISTRATION|YEAR\s*OF\s*MANUFACTURE|NAME\s*OF\s*REGISTRATION\s*AUTHORITY).*$/i, "")
    .replace(/[^\w]/g, "")
    .toUpperCase()
    .trim();
}
...
function extractNewIndiaPremiumSchedule(text) {
  ...
  const schedule = sliceText(text, /SCHEDULE OF PREMIUM/i, /GSTIN\(Issuing Office\)|Limitation as to use|Limits of Liability|Premium and GST Details/i) || text;
  ...
  result.basicThirdPartyLiability = result.basicThirdPartyLiability || amount(/Basic\s+TP\s+Premium\s+([0-9,]+(?:\.\d{1,2})?)/i);
  result.zeroDepreciationCover = result.zeroDepreciationCover || amount(/Premium\s+for\s+nil\s+depreciation\s+cover\s+([0-9,]+(?:\.\d{1,2})?)/i);
  result.engineProtectionPremium = amount(/Engine\s+Protect(?:ion)?\s+Cover\s+Premium\s+([0-9,]+(?:\.\d{1,2})?)/i);
  ...
  result.ncbPercentage = result.ncbPercentage || matchGroup(schedule, /(?:Calculated\s+NCB\s+Discount|Total\s+NCB\s+Discount)\s*\((\d{1,2}%)/i) || "";
...
function parseNewIndiaStackedOwnDamagePremiums(schedule = "") {
  ...
  const block = sliceText(schedule, /Basic\s+OD\s+Premium/i, /Calculated\s+OD\s+Premium|Total\s+OD\s+Premium|Net\s+Premium/i) || "";
...
function buildNewIndiaPremiumBreakup(schedule = "") {
  const map = {};
  const lines = String(schedule || "").split(/\n/).map((line) => line.trim()).filter(Boolean);
  const labels = [
...
function extractIffcoCompressedVehicleTable(block) {
  ...
  if (!makeModel && chassisLabelIndex !== -1) {
    ...
    if (makeLineIndex !== -1) {
      ...
      if (/-$/.test(makeLine) && /^[A-Za-z0-9]{1,8}$/.test(suffix || "") && !isPlausibleEngineNumber(suffix)) {
        makeLine += suffix;
        engineOffset = 2;
      }
...
function extractTataAigVehicleIdentifier(text, type) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    for (let offset = 1; offset <= 4 && index + offset < lines.length; offset++) {
      ...
      let compact = candidateLine.replace(type === "engine" ? /[^A-Z0-9.]/gi : /[^A-Z0-9]/gi, "").toUpperCase();
      if (type === "chassis" && compact.length < 17 && /^[A-Z0-9]{1,8}$/i.test(lines[index + offset + 1] || "")) {
...
function detectTataAigFormat(text = "") {
  if (/Standalone\s+Own\s+Damage\s+Private\s+Car\s+Policy/i.test(text)) return "TATA_AIG_STANDALONE_OD_PRIVATE_CAR_V1";
...
function extractTataAfterLabel(block = "", startLabels = [], endLabels = []) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const inline = lines[index].replace(startPattern, "").replace(/^[\s:/.-]+/, "").trim();
    const collected = [];
...
function extractTataAigMakeModelVariant(text = "") {
  const directRaw = cleanHdfcValue(matchGroup(text, /Make\s*\/\s*Model\s*\/[\s\S]*?Variant\s*\n?\s*([^\n]+)/i));
  ...
  const parts = normalized.split("/").map((part) => cleanTataVehicleValue(part)).filter(Boolean);
  let make = "";
  let model = "";
  let variant = "";
  ...
  if (parts.length >= 3) {
    ...
    const knownMakes = ["TATA MOTORS", "MARUTI SUZUKI", "MARUTI", "HYUNDAI", "HONDA", "TATA", "MAHINDRA", "TOYOTA", "FORD", "RENAULT", "NISSAN", "VOLKSWAGEN", "KIA", "MG", "SKODA"];
...
function extractTataAigWrappedVehicleValue(text = "") {
  const lines = String(text || "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  for (let index = 0; index < lines.length; index++) {
    if (!/^Make\s*\/\s*Model\s*\/?$/i.test(lines[index]) && !/^Make\s*\/\s*Model\s*\/\s*Variant$/i.test(lines[index])) continue;

    const collected = [];
    for (let offset = 1; index + offset < lines.length && offset <= 5; offset++) {
      let candidate = lines[index + offset];
      if (/^(?:Fuel Type|Engine Number|Engine No\.?|Chassis No\.?|Body Type|CC\/KW|CC \/ KW)\b/i.test(candidate)) break;
      const inlineStop = candidate.search(/\b(?:Fuel Type|Engine Number|Engine No\.?|Chassis No\.?|Body Type|CC\/KW|CC \/ KW)\b/i);
      if (inlineStop > 0) {
...
function mergeWrappedTataModel(model = "", variant = "") {
  const variantTokens = String(variant || "").split(/\s+/).filter(Boolean);
  if (!variantTokens.length) return { model, variant };

  const firstToken = variantTokens[0];
  const spacedModel = String(model || "").replace(/\bHYC\s+ROSS\b/i, "HYCROSS").trim();
...
function extractTataAigGstAmount(text = "") {
  const matches = [...String(text || "").matchAll(/(?:SGST\/UGST|SGST|CGST|IGST)\s*@?\s*\d+%?\s*₹?\s*([0-9,.]+)/gi)];
...
function extractTataAigMotor(text, _sourceFile = "") {
  ...
  let rawPolicyNumber = extractTataAigPolicyNumber(text) ||
    matchGroup(text.split(/Previous Insurance Details|Transcript Of Proposal/i)[0], /Policy No\.?\s*([0-9][0-9 ]{8,30})/i) ||
    matchGroup(text.split(/Previous Insurance Details|Transcript Of Proposal/i)[0], /Policy Number\s*(?:is\s*)?([0-9][0-9 ]{8,30})/i);
  ...
  const registrationMatch =
    matchGroup(text, /Registration No\.?\s*((?:[A-Z]{2}\s*\d{2}\s*[A-Z]{1,3}\s*\d{3,5}|[0-9]{2}\s*BH\s*\d{4}\s*[A-Z]))/i) ||
  ...
  const seatingCapacity =
    ...
    extractTataAfterLabel(text, ["Seating Capacity (Including Driver)", "Seating Capacity(Including Driver)", "Seating Capacity"], ["Vehicle Details", "Zone Details", "RTO Location"]).match(/\b\d{1,2}\b/)?.[0] ||
    "";
  ...
  const netLiabilityPremium = normalizeAmount(matchGroup(text, /Total Liability Premium\s*\([AB]\)\s*₹?\s*([0-9,.]+)/i));
  ...
  return {
    ...
    mobile,
    email,
    policyCoverType: tataFormat === "TATA_AIG_STANDALONE_OD_PRIVATE_CAR_V1" ? "Own Damage" : tataFormat === "TATA_AIG_LIABILITY_ONLY_V1" ? "Third Party" : "Package",
    gstin,
    clientId,
    bodyType,
...
function extractBajajAllianzMotor(text, _sourceFile = "") {
  ...
  return {
    documentDetected: true,
    companyName: normalizeCompanyFromMaster(/Bajaj\s+General\s+Insurance\s+Limited/i.test(text)
...
function extractGoDigitMotor(text, _sourceFile = "") {
  ...
  const companyName = normalizeCompanyFromMaster(matchGroup(text, /(Go\s+Digit\s+General\s+Insurance\s+Ltd\.)/i) || "Go Digit General Insurance Ltd.");
