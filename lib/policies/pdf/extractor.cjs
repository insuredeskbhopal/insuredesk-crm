...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  if (iciciWarehouse.documentDetected) {
    const legacyData = {
      ...
      tehsil: extractLocationPart(sourceText, iciciWarehouse.riskLocation, "tehsil"),
...
function buildWarehouseLegacyData({ sourceFile, sourceText, data, documentFormat, insuranceCompany }) {
  return {
    ...
    district: data.district || extractLocationPart(sourceText, data.riskLocation, "district"),
    tehsil: data.tehsil || extractLocationPart(sourceText, data.riskLocation, "tehsil"),
...
function isIciciWarehouseMsme(text) {
  return (
    /ICICI\s+Lombard\s+General\s+Insurance/i.test(text) &&
...
function extractTataWarehouse(text) {
  ...
  const contentsSumInsured =
    normalizeAmount(matchGroup(text, /A\.?\s*Fire[\s\S]{0,180}?([0-9][0-9,]+\.00)/i)) ||
    normalizeAmount(matchGroup(text, /Building\s*\(Refer\s+Annexure[^)]*\)\s*([0-9][0-9,]+\.00)/i));
  const burglarySumInsured = normalizeAmount(matchGroup(text, /Burglary[\s\S]{0,160}?([0-9][0-9,]+\.00)/i));
  const fidelitySumInsured =
    normalizeAmount(matchGroup(text, /Employee\s+Fidelity[\s\S]{0,160}?([0-9][0-9,]+\.00)/i)) ||
    normalizeAmount(matchGroup(text, /Annual\s+Aggregate\s*:\s*Rs\.?\s*([0-9][0-9,]+\.00)/i));
...
function extractBajajWarehouse(text) {
  ...
  const burglarySumInsured = /BURGLARY/i.test(productName)
    ...
    : "";
  const fidelitySumInsured = /FIDELITY/i.test(productName)
    ? normalizeAmount(matchGroup(text, /Per\s+Employee\s+Limit\s+Rs\s*[0-9,.]+\s*\.?\s*([0-9][0-9,]+\.00)/i))
    : "";
  const sumInsured = contentsSumInsured || burglarySumInsured || fidelitySumInsured;
  const coverages = [
...
function localExtractLocationPart(address, type) {
  ...
  if (type === "tehsil") {
    const match = cleanAddr.match(/\bTEH(?:SIL)?\b[^A-Z0-9]*([A-Z0-9]+(?:\s+[A-Z0-9]+)?)/i);
    if (match) {
      const val = match[1].trim().toUpperCase();
      if (val === "DISTRICT" || val === "DIST" || val === "STATE") return "";
      return val;
    }
  } else if (type === "district") {
    const match = cleanAddr.match(/\b(?:DIS(?:T|TRICT|CTRICT)?|DIST\.-)\b[^A-Z0-9]*([A-Z0-9]+(?:\s+[A-Z0-9]+)?)/i);
    if (match) {
      const val = match[1].trim().toUpperCase();
      if (val === "TEHSIL" || val === "TEH" || val === "STATE") return "";
      return val;
    }
  }
  return "";
...
function extractIffcoWarehouse(text, filename = "") {
  ...
  const isFire = /Flexi\s+Property\s+Protector/i.test(text) || /Fire/i.test(sourceName);
  const isBurglary = /Burglary\s+And\s+House\s+Breaking/i.test(text) || /Burglary/i.test(filename || "");
  const isFidelity = /Fidelity\s+Guarantee/i.test(text) || /Fidelity/i.test(filename || "");
  ...
  if (isFidelity) {
    ...
    let locationBlock = matchGroup(text, /Location\s*([\s\S]+?)\s*(?:Territorial Limits|Business|--|$)/i);
    if (locationBlock) {
    ...
    riskLocation = cleanIffcoWarehouseAddress(matchGroup(text, /Location address\s*([\s\S]+?)\s*Location Description/i)) || mailingAddress;
    ...
    // Premium extraction
    netPremium = normalizeIffcoWarehouseAmount(matchGroup(text, /Net Premium \(Rs\.\) \/Taxable Value\s*([\d,.]+)/i));
    ...
    premiumIncludingGst = normalizeIffcoWarehouseAmount(matchGroup(text, /Total Premium Payable \(Rs\.\)\s*([\d,.]+)/i));
  ...
  const district = localExtractLocationPart(riskLocation, "district");
  const tehsil = localExtractLocationPart(riskLocation, "tehsil");
