const FALLBACK_PATTERNS = {
  ...
  vehicleNumber: [
    ...
    /\b([A-Z]{2}[-\s]?\d{1,2}[-\s]?\d{4})\b/i,
    /(?:registration|vehicle)\s*(?:no|number)?\.?\s*([A-Z]{2}[-\s]?\d{1,2}(?:[-\s]?[A-Z]{1,3})?[-\s]?\d{4})/i,
  ],
  gstNumber: [/\b(\d{2}[A-Z]{5}\d{4}[A-Z][A-Z0-9]Z[A-Z0-9])\b/i],
  mobileNumber: [/\b([6-9]\d{9})\b/],
  email: [/\b([A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,})\b/i],
  insuredName: [/name\s+of\s+the\s+insured\s*([A-Z0-9/&().,\-\s]{3,160}?)\s*policy\s*no/i],
  policyPeriod: [
  ...
  riskLocation: [
    ...
    /insured\s+address\s*([A-Z0-9/&().,\-\s]{10,360}?)\s*(?:contact|phone|email|gstin|mobile)/i,
  ],
  businessOfInsured: [/business\s+of\s+the\s+insured\s*([A-Z0-9/&().,\-\s]{10,300}?)\s*issued\s+at/i],
  issuedAt: [/issued\s+at\s*([A-Z ]{3,80})\s*premises/i],
  brokerName: [
  ...
  policyType: [
    /policy\s+schedule\s+cum\s+certificate\s+of\s+insurance\s+((?:Private\s+Car|Two\s+Wheeler|Commercial\s+Vehicle|Motor)\s+(?:Package|Liability|Comprehensive|Bundled)\s+Policy)/i,
    /\b((?:Private\s+Car|Two\s+Wheeler|Commercial\s+Vehicle|Motor)\s+(?:Package|Liability|Comprehensive|Bundled)\s+Policy)\b/i,
  ],
  policyCoverType: [
    /applicable\s+to\s+(Liabi?li?ty\s+Only|Comprehensive|Package|Third\s+Party(?:\s+Only)?)\s+polic/i,
    /\b(Liabi?li?ty\s+Only|Comprehensive|Package|Third\s+Party(?:\s+Only)?)\s+polic/i,
  ],
  rtoLocation: [
    /name\s+of\s+registration\s*(?:authority)?\s*([A-Za-z ]{2,50}?)\s*(?:geographical|cover\s+note|financier)/i,
  ],
};
...
function extractAfterAlias(text, alias, fieldType) {
  ...
  const pattern = new RegExp(
    `${safeAlias}\\s*(?:[:\\-]|no\\.?|number)?\\s*([A-Z0-9@.,/&()\\-\\s]{2,180})`,
    "i",
  );
...
function fallbackForField(text, field) {
  ...
  if (key.includes("policynumber") || label.includes("policy number"))
    return firstMatch(text, FALLBACK_PATTERNS.policyNumber);
  if (key.includes("insuredname") || label.includes("insured name"))
    return firstMatch(text, FALLBACK_PATTERNS.insuredName);
  ...
  if (key.includes("risklocation") || label.includes("risk location") || label.includes("premises"))
    return firstMatch(text, FALLBACK_PATTERNS.riskLocation);
  ...
  if (key.includes("premium") || label.includes("premium"))
    return firstMatch(text, FALLBACK_PATTERNS.premium);
  ...
  if (key.includes("suminsured") || label.includes("sum insured"))
    return firstMatch(text, FALLBACK_PATTERNS.sumInsured);
  if (key.includes("vehiclenumber") || label.includes("vehicle number"))
    return firstMatch(text, FALLBACK_PATTERNS.vehicleNumber);
  if (
    ...
    return firstMatch(text, FALLBACK_PATTERNS.vehicleNumber);
  if (key.includes("policytype") || label.includes("policy type"))
    return firstMatch(text, FALLBACK_PATTERNS.policyType);
  if (key.includes("policycovertype") || key.includes("covertype") || label.includes("cover type"))
    return firstMatch(text, FALLBACK_PATTERNS.policyCoverType);
  if (key.includes("rtolocation") || label.includes("rto") || label.includes("registration authority"))
    return firstMatch(text, FALLBACK_PATTERNS.rtoLocation);
  if (key.includes("gst")) return firstMatch(text, FALLBACK_PATTERNS.gstNumber);
  if (key.includes("mobile") || key.includes("contact"))
    return firstMatch(text, FALLBACK_PATTERNS.mobileNumber);
