...
async function syncPolicyRenewal(policy, actorId) {
  ...
  const expiryDate = parseBusinessDate(payload.expiryDate || payload.policyEndDate || payload["Policy End Date"]);
  ...
  const customerName = payload.insuredName || payload.customerName || payload["Insured Name"] || "Unnamed customer";
  const company = payload.insuranceCompany || payload.companyName || policy.selectedCompany || "Insurance company";
  ...
  await upsertTaskWithCalendar({
    ...
    title: interval === 0 ? `Renewal due today: ${customerName}` : `${interval}-day renewal follow-up: ${customerName}`,
...
async function upsertTaskWithCalendar(input) {
  ...
  await upsertNotification({
    ...
    createdById: input.createdById,
    category: "TASK",
    severity: input.priority === "CRITICAL" || input.priority === "EMERGENCY" ? "CRITICAL" : input.priority === "HIGH" ? "WARNING" : "INFO",
    title: "Task assigned",
    message: input.title,
    module: input.module,
    recordId: input.recordId,
...
export async function getOperationsSummary(session) {
  ...
  const [
    ...
    prisma.task.count({ where: { ...openWhere, type: "RENEWAL", dueAt: { gte: todayStart, lte: todayEnd } } }),
    ...
    prisma.calendarEvent.count({ where: { ...tenantWhere, eventType: "MEETING", startsAt: { gte: todayStart, lte: todayEnd } } }),
...
async function getTeamWorkload(session) {
  ...
  const users = await prisma.user.findMany({
    where: session.role === "SUPER_ADMIN" && !session.organizationId
      ? { deletedAt: null }
  ...
  return users.map((user) => {
    ...
    const completed = userTasks.filter((task) => task.status === "COMPLETED" || task.status === "CLOSED").length;
    const overdue = userTasks.filter((task) => OPEN_TASK_STATUSES.includes(task.status) && task.dueAt && startOfDay(task.dueAt) < now).length;
    return {
      userId: user.id,
