...
async function extractPdfText(buffer) {
  try {
    ...
    throw new Error(`PDF text extraction failed: ${error instanceof Error ? error.message : "Corrupt or unreadable PDF."}`);
  }
}
...
async function runOcrFallback(buffer) {
  try {
    ...
    if (!pageImages.length) {
      return { rawText: "", extractionMethod: "failed", pages: 0, warnings: ["No PDF pages could be rendered for OCR."] };
    }
...
function isTextQualityAcceptable(text) {
  ...
  const policySignals = ["policy", "insured", "premium", "sum insured", "expiry"].filter((signal) => normalized.toLowerCase().includes(signal)).length;
