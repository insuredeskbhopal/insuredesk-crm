...
export function canAccessResource(user, action, resourceOwnerId, resourceOrgId) {
  ...
  switch (user.role) {
    case UserRole.ADMIN:
      // ADMIN has full read, write, delete permissions within their organization
      return true;
    ...
    case UserRole.AGENT:
      // AGENT can read/write only their own created/owned records
...
export function getTenantFilter(user, action = "read") {
  ...
  // AGENT is restricted further: can only read/write their own records
  if (user.role === UserRole.AGENT) {
    baseFilter.createdById = user.id;
  }
