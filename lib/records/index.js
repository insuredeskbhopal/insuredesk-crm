...
export function normalizeRecord(record) {
  ...
  const insuredName = payload.insuredName || legacy.insuredName || payload.customerName || legacy.customerName || payload["Insured Name"] || "";
  const contactNumber =
  ...
  const insuranceCompany = normalizeInsuranceCompanyName(
    payload.insuranceCompany || payload.insurerName || legacy.insuranceCompany || payload["Insurance Company"] || record.selectedCompany || record.detectedCompany || ""
  );

  const renewalRemarks = Array.isArray(payload.renewalRemarks) ? payload.renewalRemarks : Array.isArray(legacy.renewalRemarks) ? legacy.renewalRemarks : [];
  ...
  const assignedTo = payload.assignedTo || legacy.assignedTo || renewalFollowUp?.assignedTo || createdByName || "";
  ...
  return {
    ...
    contactPerson:
      ...
      "",
    email: payload.email || legacy.email || payload.customerEmail || legacy.customerEmail || payload.emailAddress || legacy.emailAddress || payload["Email ID"] || payload["Email Address"] || "",
    ...
    sourceDocumentType: payload.sourceDocumentType || legacy.sourceDocumentType || payload.documentFormat || legacy.documentFormat || "",
    productName: payload.productName || legacy.productName || "",
    mailingAddress: payload.mailingAddress || legacy.mailingAddress || payload.communicationAddress || legacy.communicationAddress || payload.address || legacy.address || "",
    premisesAddress: payload.premisesAddress || legacy.premisesAddress || payload.riskLocation || legacy.riskLocation || "",
    businessDescription: payload.businessDescription || legacy.businessDescription || payload.description || legacy.description || "",
    ...
    premiumIncludingGst: payload.premiumIncludingGst || legacy.premiumIncludingGst || payload.totalPremium || legacy.totalPremium || payload.premium || legacy.premium || "",
    ...
    coverages: Array.isArray(payload.coverages) ? payload.coverages : Array.isArray(legacy.coverages) ? legacy.coverages : [],
    clauses: Array.isArray(payload.clauses) ? payload.clauses : Array.isArray(legacy.clauses) ? legacy.clauses : [],
    specialConditions: Array.isArray(payload.specialConditions) ? payload.specialConditions : Array.isArray(legacy.specialConditions) ? legacy.specialConditions : [],
