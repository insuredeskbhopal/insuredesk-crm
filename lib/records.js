export function normalizeRecord(record) {
  ...
  const bankName =
    payload.bankName ||
    legacy.bankName ||
    payload.selectedBankSource ||
    legacy.selectedBankSource ||
    record.selectedBankSource ||
    payload.detectedBankSource ||
    legacy.detectedBankSource ||
    record.detectedBankSource ||
    payload["Bank Name"] ||
    payload.Bank ||
    "";
...
export function buildCustomerId(insuredName = "", contactNumber = "") {
  const namePart = String(insuredName || "")
    .replace(/[^A-Za-z]/g, "")
    .slice(0, 4)
    .toUpperCase();
