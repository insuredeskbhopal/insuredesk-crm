import { validatePolicyCrossFields, validatePolicyFields } from "./policy-validation-engine.cjs";
...
export function buildAiMergePreview({
  ...
  const suspiciousFields = new Set([...(aiReview.suspiciousFields || []), ...getValidationFields(validation)]);
  ...
  for (const [field, suggestion] of Object.entries(suggestions)) {
    ...
    const reason = suggestion.reason || buildEligibilityReason({ fieldIsBlank, fieldFailedValidation, fieldIsSuspicious });
...
function containsSourceSnippet(sourceText, evidenceText) {
  const source = String(sourceText || "").replace(/\s+/g, " ").trim().toLowerCase();
  const snippet = String(evidenceText || "").replace(/\s+/g, " ").trim().toLowerCase();
...
function evidenceSupportsSuggestedValue(suggestedValue, evidenceText) {
  ...
  return suggestionTokens.filter((token) => evidence.includes(token)).length >= Math.min(2, suggestionTokens.length);
}
