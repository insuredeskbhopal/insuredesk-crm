...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  const legacyData = {
    ...
    netPremium: hdfcErgoMotor.netPremium || newIndiaPremiumSchedule.netPremium,
    tpDriverOwner: newIndiaPremiumSchedule.tpDriverOwner,
    odPremium: newIndiaPremiumSchedule.odPremium,
...
function extractNewIndiaPremiumSchedule(text) {
  ...
  const amount = (pattern) => normalizeAmount(matchGroup(schedule, pattern));

  return {
    odPremium: amount(/Total\s+OD\s+Premium\s*([0-9,]+(?:\.\d{1,2})?)/i) ||
      amount(/Calculated\s+OD\s+Premium\s*([0-9,]+(?:\.\d{1,2})?)/i),
    tpDriverOwner: amount(/Total\s+TP\s+Premium\s*([0-9,]+(?:\.\d{1,2})?)/i) ||
      amount(/Calculated\s+TP\s+Premium\s*([0-9,]+(?:\.\d{1,2})?)/i),
    netPremium: amount(/Net\s+Premium\s+in\s+Rs\s*([0-9,]+(?:\.\d{1,2})?)/i),
    gstAmount: amount(/GST\s+in\s+Rs\s*([0-9,]+(?:\.\d{1,2})?)/i),
    totalPremium: amount(/Total\s+Payable\s+in\s+Rs\s*([0-9,]+(?:\.\d{1,2})?)/i)
  };
}
