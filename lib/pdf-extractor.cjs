...
function extractNewIndiaPremiumSchedule(text) {
  ...
  if (!netPremium && totalPremium && gstAmount) netPremium = diffAmounts(totalPremium, gstAmount);
  if (!gstAmount && totalPremium && netPremium) gstAmount = diffAmounts(totalPremium, netPremium);
  if (!odPremium && netPremium && tpDriverOwner) odPremium = diffAmounts(netPremium, tpDriverOwner);
  if (!tpDriverOwner && netPremium && odPremium) tpDriverOwner = diffAmounts(netPremium, odPremium);
