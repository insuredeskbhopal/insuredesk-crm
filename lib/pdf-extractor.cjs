...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  const legacyData = {
    ...
    premium: hdfcErgoMotor.totalPremium || generaliMotor.totalPremium || newIndiaPremiumSchedule.totalPremium || premium,
    totalPremium: hdfcErgoMotor.totalPremium || generaliMotor.totalPremium || newIndiaPremiumSchedule.totalPremium,
    netPremium,
    tpDriverOwner,
    odPremium,
    ...
    basicOwnDamage: hdfcErgoMotor.basicOwnDamage || generaliMotor.basicOwnDamage || newIndiaPremiumSchedule.basicOwnDamage,
    ...
    netOwnDamagePremium: hdfcErgoMotor.netOwnDamagePremium || generaliMotor.netOwnDamagePremium || newIndiaPremiumSchedule.odPremium,
    netLiabilityPremium: hdfcErgoMotor.netLiabilityPremium || generaliMotor.netLiabilityPremium || newIndiaPremiumSchedule.tpDriverOwner,
    totalPackagePremium: hdfcErgoMotor.totalPackagePremium || generaliMotor.totalPackagePremium || newIndiaPremiumSchedule.netPremium,
    gstAmount: hdfcErgoMotor.gstAmount || generaliMotor.gstAmount || newIndiaPremiumSchedule.gstAmount,
    dueCollection: paymentCollection.dueCollection,
    collectedAmount: paymentCollection.collectedAmount,
...
function shouldKeepExtractedMotorVariant(data = {}, policyUnderstanding = {}, policySchema = {}) {
  ...
  return /\bnew\s+india\b|\bTATA\s*AIG\b/i.test(haystack);
}
...
function extractNewIndiaPremiumSchedule(text) {
  ...
  result.odPremium = amount(
    /Total\s+OD\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Calculated\s+OD\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /OD\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Own\s+Damage\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i
  );
  result.tpDriverOwner = amount(
    /Total\s+TP\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Calculated\s+TP\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /TP\s*\+\s*Driver\s*\+\s*Owner\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /TP\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Liability\s+Premium\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i
  );
  result.netPremium = amount(
    /Net\s+Premium\s+in\s+Rs\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Net\s+Premium\s*[:.-]?\s*(?:Rs\.?)?\s*([0-9,]+(?:\.\d{1,2})?)/i
  );
  result.gstAmount = amount(
    /GST\s+in\s+Rs\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /GST\s*[:.-]?\s*(?:Rs\.?)?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Goods\s+and\s+Service\s+Tax\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i
  );
  result.totalPremium = amount(
    /Total\s+Payable\s+in\s+Rs\s*[:.-]?\s*([0-9,]+(?:\.\d{1,2})?)/i,
    /Total\s+Premium\s*[:.-]?\s*(?:Rs\.?)?\s*([0-9,]+(?:\.\d{1,2})?)/i,
...
function extractTataAigMotor(text, _sourceFile = "") {
  ...
  const rawPolicyNumber = matchGroup(text, /Policy No\.?\s*([0-9][0-9 ]{8,30})/i).replace(/\s+/g, " ").trim() ||
    matchGroup(text, /Policy Number\s*(?:is\s*)?([0-9][0-9 ]{8,30})/i).replace(/\s+/g, " ").trim();
  ...
  const registrationNumber = matchGroup(text, /Registration No\.?\s*([A-Z]{2}\s?\d{2}\s?[A-Z]{1,3}\s?\d{4})/i).toUpperCase();
  ...
  const rawEngine = matchGroup(text, /Engine Number\s*\/[\s\S]*?Motor No\.\s*\(for EV\)\s*\n?\s*([A-Z0-9]{3,25})/i);
  const engineNumber = rawEngine.toUpperCase().replace(/[^A-Z0-9]/g, "");

  const rawChassis = matchGroup(text, /Chassis No\S*\s*([A-Z0-9]{17})/i);
  const chassisNumber = rawChassis.toUpperCase().replace(/[^A-Z0-9]/g, "");
