...
} = require("./policy-parser-registry.cjs");
const { validateExtractionNonDestructive } = require("./policy-validation-engine.cjs");
...
let policyParserRegistry = null;

function getPolicyParserRegistry() {
  if (policyParserRegistry) return policyParserRegistry;

  policyParserRegistry = createParserRegistry([
    {
      id: "ICICI_LOMBARD_MOTOR",
      companyName: "ICICI Lombard General Insurance Company Limited",
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bICICI\s+Lombard\s+General\s+Insurance\s+Company\s+Limited\b/i,
        /\bICICI\s+Lombard\b/i
      ]),
      extract: (document) => extractIciciMotor(document.text, document.sourceFile)
    },
    {
      id: "TATA_AIG_MOTOR",
      ...
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bTATA\s+AIG\s+General\s+Insurance\s+Company\s+Limited\b/i,
        /\bTATA\s+AIG\b/i
      ]),
      extract: (document) => extractTataAigMotor(document.text, document.sourceFile)
    },
    {
      id: "ROYAL_SUNDARAM_MOTOR",
      companyName: "Royal Sundaram General Insurance Co. Limited",
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bRoyal\s+Sundaram\s+General\s+Insurance\b/i,
        /\bRoyal\s+Sundaram\b/i
      ]),
      extract: (document) => extractRoyalSundaramMotor(document.text, document.sourceFile)
    },
    {
      id: "BAJAJ_ALLIANZ_MOTOR",
      companyName: "Bajaj Allianz General Insurance Company Limited",
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bBajaj\s+Allianz\s+General\s+Insurance\b/i,
        /\bBajaj\s+General\s+Insurance\s+Limited\b/i
      ]),
      extract: (document) => extractBajajAllianzMotor(document.text, document.sourceFile)
    },
    {
      id: "GO_DIGIT_MOTOR",
      companyName: "Go Digit General Insurance Ltd.",
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bGo\s+Digit\s+General\s+Insurance\b/i,
        /\bDigit\s+General\s+Insurance\b/i
      ]),
      extract: (document) => extractGoDigitMotor(document.text, document.sourceFile)
    },
    {
      id: "HDFC_ERGO_MOTOR",
      ...
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bHDFC\s+ERGO\s+General\s+Insurance\s+Company\s+Limited\b/i,
        /\bHDFC\s+ERGO\b/i
      ]),
      extract: (document) => extractHdfcErgoMotor(document.text)
    },
    {
      id: "GENERALI_MOTOR",
      ...
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bGenerali\s+Central\s+Insurance\s+Company\s+Limited\b/i,
        /\bFuture\s+Generali\s+India\s+Insurance\s+Company\s+Limited\b/i
      ]),
      extract: (document) => extractGeneraliMotor(document.text)
    },
    {
      id: "NEW_INDIA_MOTOR",
      ...
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bThe\s+New\s+India\s+Assurance\s+Company\s+Limited\b/i,
        /\bTHE\s+NEW\s+INDIA\s+ASSURANCE\s+CO\.?\s+LTD\.?\b/i,
        /\bNew\s+India\s+Assurance\b/i
      ]),
      extract: (document) => extractRegisteredNewIndiaMotor(document.text)
    },
    {
      id: "IFFCO_TOKIO_MOTOR",
      companyName: "IFFCO-TOKIO GENERAL INSURANCE CO.LTD",
      policyType: "Motor Insurance",
      detect: (document) => detectRegisteredCompany(document, [
        /\bIFFCO[-\s]?TOKIO\s+GENERAL\s+INSURANCE\b/i,
        /\bIFFCO\s+TOKIO\s+GEN\s+INSU\.?\s+CO\.?\s+LTD\.?\b/i,
        /\bIFFCO[-\s]?TOKIO\b/i
      ]),
      extract: (document) => extractRegisteredIffcoTokioMotor(document.text)
    }
  ]);

  return policyParserRegistry;
}

function extractRegisteredNewIndiaMotor(text) {
  const fields = extractNewIndiaMotorDetails(text);
  const hasMotorSignal = /\b(?:private\s+car|two\s+wheeler|commercial\s+vehicle|package\s+policy|liability\s+policy|registration\s+no|engine\s+no|chassis\s+no)\b/i.test(text);

  return {
    documentDetected: hasMotorSignal,
    companyName: "The New India Assurance Company Limited",
    policyType: fields.policyType || "Motor Insurance",
    ...fields
  };
}

function extractRegisteredIffcoTokioMotor(text) {
  const hasMotorSignal = /\b(?:insured\s+motor\s+vehicle\s+details|registration\s+mark|engine\s+no|chassis\s+no|P400\s+Policy|private\s+car|two\s+wheeler)\b/i.test(text);

  return {
    documentDetected: hasMotorSignal,
    companyName: "IFFCO-TOKIO GENERAL INSURANCE CO.LTD",
    policyType: matchGroup(text, /\b(?:Private\s+Car|Two\s+Wheeler|Commercial\s+Vehicle)[^\n]{0,80}Policy\b/i) || "Motor Insurance",
...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  const parserSelection = runParserRegistry(getPolicyParserRegistry(), {
    text: sourceText,
    sourceFile,
    understanding: policyUnderstanding
  });

  const iciciMotor = getRegisteredParserResult(parserSelection, "ICICI_LOMBARD_MOTOR") || extractIciciMotor(sourceText, sourceFile);
  ...
  const tataAigMotor = getRegisteredParserResult(parserSelection, "TATA_AIG_MOTOR") || extractTataAigMotor(sourceText, sourceFile);
  ...
  const royalSundaramMotor = getRegisteredParserResult(parserSelection, "ROYAL_SUNDARAM_MOTOR") || extractRoyalSundaramMotor(sourceText, sourceFile);
  ...
  const bajajAllianzMotor = getRegisteredParserResult(parserSelection, "BAJAJ_ALLIANZ_MOTOR") || extractBajajAllianzMotor(sourceText, sourceFile);
  ...
  const goDigitMotor = getRegisteredParserResult(parserSelection, "GO_DIGIT_MOTOR") || extractGoDigitMotor(sourceText, sourceFile);
  ...
  const hdfcErgoMotor = getRegisteredParserResult(parserSelection, "HDFC_ERGO_MOTOR") || extractHdfcErgoMotor(sourceText);
  const generaliMotor = getRegisteredParserResult(parserSelection, "GENERALI_MOTOR") || extractGeneraliMotor(sourceText);
  ...
  const fuelType =
    ...
    matchGroup(sourceText, /Type\s+of\s+body\s*\/\s*Type\s+of\s+Fuel[A-Z ]*\/\s*(Petrol|Diesel|CNG|Electric|EV|LPG|Hybrid)\b/i) ||
...
function buildIntelligentResult(legacyData, policyUnderstanding, policySchema, schemaExtraction) {
  ...
  const nonDestructiveValidation = validateExtractionNonDestructive(mergedData);
  ...
  return {
    ...
    extractionQuality: {
      ...
      validation: nonDestructiveValidation,
      aiReview: buildPassiveAiReviewPlaceholder(mergedData, nonDestructiveValidation),
      aiMergePreview: buildAiMergePreviewPlaceholder()
    },
    policyUnderstanding,
    schemaExtraction: {
...
function buildAiMergePreviewPlaceholder() {
  return {
    eligibleUpdates: {},
    blockedUpdates: {},
    reasons: {}
...
function buildPassiveAiReviewPlaceholder(data = {}, validation = {}) {
  return {
    provider: "",
    model: "",
    used: false,
    ...
    detectedCompany: data.insuranceCompany || data.companyName || "",
...
function extractPolicyNumber(text) {
  const iffcoPolicyNumber = matchGroup(text, /P400\s+Policy\s*#\s*:?\s*([A-Z]\d{5,})\b/i);
  if (iffcoPolicyNumber) return iffcoPolicyNumber;
