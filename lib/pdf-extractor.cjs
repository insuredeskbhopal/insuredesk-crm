...
function extractTataAigMotor(text, _sourceFile = "") {
  ...
  const rawEngine = matchGroup(text, /Engine Number\s*[:/]?\s*([A-Z0-9-]{3,25})/i) || matchGroup(text, /Motor No\.\s*\(for EV\)\s*\n?\s*([A-Z0-9-]{3,25})/i) || "";
  const engineNumber = rawEngine ? String(rawEngine).toUpperCase().replace(/[^A-Z0-9]/g, "") : "";

  const rawChassis = matchGroup(text, /Chassis No\S*\s*([A-Z0-9-]{10,25})/i) || matchGroup(text, /Chassis\s*No[:\s]*([A-Z0-9-]{10,25})/i) || "";
