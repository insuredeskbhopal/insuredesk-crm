...
function isTataAigMotor(text) {
  const signals = [
    /TATA\s*AIG/i,
    /customersupport@tataaig\.com/i,
    /Auto\s*Secure/i,
    /Private\s*Car\s*Package\s*Policy/i,
    /Motor\s*Insurance/i,
    /tata\s*aig\s*general\s*insurance/i
  ];
  let matches = 0;
...
function extractTataAigMotor(text, _sourceFile = "") {
  ...
  const rawPolicyNumber = matchGroup(text, /Policy No\.?\s*([0-9\s]{10,24})/i).replace(/\s+/g, " ").trim() ||
                          matchGroup(text, /Policy Number\s*([0-9\s]{10,24})/i).replace(/\s+/g, " ").trim();
  const policyNumber = rawPolicyNumber.replace(/\s+/g, "");

  const insuredName = extractTataInsuredName(text);
  const startDate = normalizeTataDate(
    matchGroup(text, /Own Damage Cover\s*(\d{2}\/\d{2}\/\d{4})/i) ||
    matchGroup(text, /TP Cover Period\s*[:.-]?\s*(\d{2}\/\d{2}\/\d{4})/i)
  );
  const expiryDate = normalizeTataDate(
    matchGroup(text, /Own Damage Cover\s*\d{2}\/\d{2}\/\d{4}[^\n]*?(\d{2}\/\d{2}\/\d{4})/i) ||
    matchGroup(text, /TP Cover Period\s*[:.-]?\s*\d{2}\/\d{2}\/\d{4}[^\n]*?to\s*(\d{2}\/\d{2}\/\d{4})/i)
  );

  const registrationNumber = matchGroup(text, /Registration No\S*\s*([A-Z]{2}\s?\d{2}\s?[A-Z]{1,2}\s?\d{4})/i).toUpperCase().replace(/\s+/g, "");
  ...
  const registrationDate = normalizeTataDate(matchGroup(text, /Date of Registration\s*([0-9/]{8,10})/i));
  ...
  const financer = matchGroup(text, /Hypothecation\s*\/\s*Lease\s*with\s*\n?\s*([A-Za-z0-9 ]+)/i);
  ...
  const gstin = matchGroup(text, /GSTIN\s*([A-Z0-9]{15})/i);
