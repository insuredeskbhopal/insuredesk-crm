...
function extractPolicyFromText(text, sourceFile = "") {
  ...
  const legacyData = {
    ...
    sumInsured: hdfcErgoMotor.totalIdv || generaliMotor.totalIdv || motorVehicleTable.idv || sumInsured,
    ...
    grossVehicleWeight,
    idv: hdfcErgoMotor.totalIdv || generaliMotor.totalIdv || motorVehicleTable.idv || idv,
...
function extractEngineNumber(text) {
  ...
  const newIndiaChassisEngine = text.match(/Chassis\s*no\.?\s*\/\s*Engine\s*(?:no\.?|Number)\s*:?\s*([A-Z0-9]{17})\s*\/\s*([A-Z0-9\n ]{3,30}?)(?=\s*(?:Type|Make|Registration|Model|Fuel|Year|$))/i);
  if (newIndiaChassisEngine?.[2]) {
    const engine = newIndiaChassisEngine[2].replace(/[\r\n\s]+/g, "").trim().toUpperCase();
    if (isPlausibleEngineNumber(engine)) return engine;
  }
