...
function shouldKeepExtractedMotorVariant(data = {}, policyUnderstanding = {}, policySchema = {}) {
  ...
  return /\bnew\s+india\b|\bTATA\s*AIG\b|\bICICI\s+Lombard\b/i.test(haystack);
}
...
function extractGenericPremiumSchedule(text, fallbackTotal) {
  ...
  const od = 
    matchGroup(text, /\bNet\s*\(A\)\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bTotal\s*OD\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bOD\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bOwn\s*Damage\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bOwn\s*Damage\s*\(A\)\s*[:.-]?\s*([0-9,]+\.\d{2})/i);

  const tp = 
    matchGroup(text, /\bNet\s*\(B\)\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bTotal\s*TP\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bTP\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bThird\s*Party\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bLiability\s*Premium\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bTP\s*\+\s*Driver\s*\+\s*Owner\s*[:.-]?\s*([0-9,]+\.\d{2})/i);

  const net = 
    matchGroup(text, /\bSection\s*1\s*\(A\s*\+\s*B\)\s*(?:\(for\s*1\s*years?\)?)?\s*(?:Rs\.?)?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bNet\s+Premium\s*(?:Rs\.?)?\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bNet\s+Premium\s*\(A\+B\+C\+D\)\s*₹?\s*([0-9,]+\.\d{2})/i);

  const total = 
    matchGroup(text, /\bTotal\s*Premium\s*(?:inclusive\s*Tax)?\s*(?:\(?Rs\.?\)?)?\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bTotal\s*Payable\s*[:.-]?\s*([0-9,]+\.\d{2})/i) ||
    matchGroup(text, /\bPremium\s+including\s+GST\s*[:.-]?\s*([0-9,]+\.\d{2})/i);
  ...
  if (result.totalPremium && result.netPremium) {
