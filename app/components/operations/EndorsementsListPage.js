...
const STATUSES = ["All", "Draft", "Letter Generated", "Sent to Customer", "Pending Insurance Company Letter", "Insurance Company Letter Received", "Approved", "Rejected", "Cancelled"];
    ...
    <div className="endorsement-page">
      ...
      <form className="endorsement-filter-bar" onSubmit={applyFilters}>
        <label className="endorsement-search-field">
          <Search size={17} />
          <input value={filters.q} onChange={(event) => updateFilter("q", event.target.value)} placeholder="Search policy, customer, endorsement no., company" />
        </label>
        ...
        <input type="date" value={filters.from} onChange={(event) => updateFilter("from", event.target.value)} aria-label="From date" />
        <input type="date" value={filters.to} onChange={(event) => updateFilter("to", event.target.value)} aria-label="To date" />
        <button type="submit">Apply</button>
      </form>

      {error ? <div className="endorsement-alert"><span>{error}</span><button type="button" onClick={() => loadRecords()}>Retry</button></div> : null}
            ...
            <table className="endorsement-table">
              ...
              <tbody>
                {records.map((record, index) => (
                  <tr key={record.id}>
                    <td>{index + 1}</td>
                    <td><strong>{record.endorsementNo}</strong></td>
                    ...
                    <td><span className={`endorsement-status ${statusClass(record.status)}`}>{record.status}</span></td>
                    ...
                    <td>
                      <div className="endorsement-actions">
                        <Link href={`/dashboard/endorsements/${record.id}`} title="View"><Eye size={15} /></Link>
                        <Link href={`/dashboard/endorsements/${record.id}?mode=edit`} title="Edit"><Edit size={15} /></Link>
                        {record.generatedLetterPdfUrl ? <a href={record.generatedLetterPdfUrl} download={record.generatedLetterFileName || `${record.endorsementNo}.pdf`} title="Download Letter PDF"><Download size={15} /></a> : null}
                        <Link href={`/dashboard/endorsements/${record.id}?upload=company-letter`} title="Upload Insurance Company Letter"><Upload size={15} /></Link>
                        <button type="button" disabled={savingId === record.id} onClick={() => updateStatus(record, "Approved")} title="Mark as Approved"><CheckCircle2 size={15} /></button>
                        <button type="button" disabled={savingId === record.id} onClick={() => updateStatus(record, "Rejected")} title="Mark as Rejected / Cancelled"><XCircle size={15} /></button>
                        {record.status === "Draft" ? (
                          <button type="button" disabled={savingId === record.id} onClick={() => deleteDraft(record)} title="Delete draft"><Trash2 size={15} /></button>
                        ) : null}
                      </div>
                    </td>
                  </tr>
