  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      ...
      <div className="renewals-grid" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))" }}>
        {[
          { key: "all_work", title: "All Daily Tasks", count: counts.dueToday + counts.followUpToday + counts.overdueFollowUp, color: "var(--rn-primary)" },
          ...
          { key: "overdue_followup", title: "Overdue Follow-ups", count: counts.overdueFollowUp, color: "var(--rn-danger)" },
          { key: "completed_today", title: "Completed Today", count: counts.completedToday, color: "var(--rn-success)" }
        ].map((card) => (
          <div 
            key={card.key}
          ...
          >
            <div>
              <h4 className="renewals-card-title" style={{ fontSize: "11px", marginBottom: "8px" }}>{card.title}</h4>
              <p className="renewals-card-value" style={{ fontSize: "22px" }}>{card.count}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rn-table-container">
        <div style={{ padding: "16px", borderBottom: "1px solid var(--rn-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontSize: "12px", color: "var(--rn-text-secondary)" }}>{filtered.length} records found</span>
        </div>
            ...
            <tbody>
              {filtered.map((policy) => {
                ...
                return (
                  <tr key={policy.id}>
                    <td>
                      <div style={{ fontWeight: "600", color: "var(--rn-text-primary)" }}>{policy.insuredName}</div>
                      <div style={{ fontSize: "12px", color: "var(--rn-text-muted)" }}>{policy.contactNumber || "No Mobile"}</div>
                    </td>
                    ...
                    <td>
                      ...
                      {daysLeft < 0 ? (
                        <span style={{ fontSize: "11px", color: "var(--rn-danger)", fontWeight: "500" }}>Overdue {Math.abs(daysLeft)} days</span>
                      ) : (
                        <span style={{ fontSize: "11px", color: "var(--rn-text-muted)" }}>{daysLeft} days left</span>
                      )}
                    </td>
                    <td>
                      <div style={{ fontWeight: "500" }}>{policy.renewalFollowUp?.nextAction || "-"}</div>
                      {policy.nextFollowUpDate && (
                        <span style={{ fontSize: "11px", color: "var(--rn-primary)" }}>F/Up: {policy.nextFollowUpDate}</span>
                      )}
                    </td>
                    <td>{policy.assignedTo || "Unassigned"}</td>
                    <td>
                      <span className={`rn-badge ${
                        policy.renewalStatus === "RENEWED" ? "rn-badge-success" :
                        ["LOST", "NOT_INTERESTED", "WRONG_NUMBER", "RENEWED_ELSEWHERE"].includes(policy.renewalStatus) ? "rn-badge-danger" :
                        policy.renewalStatus === "Follow-Up" ? "rn-badge-warning" : "rn-badge-active"
                      }`}>
