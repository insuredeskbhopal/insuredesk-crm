    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Generate and manage Client IDs and secure customer portal access.
          </p>
        </div>
        <button
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-slate-600 space-y-1">
          ...
          <p>
            * <strong>Client ID</strong>: This is the unique profile identification key generated
            automatically for the client.
          </p>
          <p>
            * <strong>Client MPIN</strong>: Treat portal credentials as private. Clients can change their MPIN
            securely inside the portal.
          </p>
        </div>
      </div>
      ...
      {temporaryCredential && (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-bold">Temporary portal credential for {temporaryCredential.name}</p>
              <p className="mt-1 text-xs text-amber-800">
                Share this once through a private channel. It is not stored in readable form.
              </p>
              <div className="mt-3 flex flex-wrap items-center gap-2 font-mono text-xs">
                <span className="rounded-lg bg-white px-3 py-2">
                  Client ID: {temporaryCredential.clientId}
                </span>
                <span className="rounded-lg bg-white px-3 py-2 text-base font-black tracking-[0.22em]">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-amber-300 bg-white px-3 py-2 font-sans font-bold"
                >
                  ...
                  {copiedId === `credential-${temporaryCredential.clientId}` ? "Copied" : "Copy credential"}
                </button>
              </div>
            </div>
            <button
              ...
              aria-label="Hide temporary credential"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">
                  Review pending requests and correct those returned by Super Admin.
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div key={item.id} className="grid gap-3 p-4 md:grid-cols-[1fr_auto] md:items-center">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${item.status === "WAITING_DOCUMENTS" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}
                        >
                      ...
                      {item.correctionNote && (
                        <p className="mt-2 rounded-lg border border-rose-100 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">
                          Super Admin note: {item.correctionNote}
                        </p>
                      )}
                    </div>
                    {item.status === "WAITING_DOCUMENTS" && (
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-lg bg-sky-600 px-4 py-2 text-xs font-bold text-white hover:bg-sky-700"
                      >
          ...
          <div className="p-4 border-b border-amber-100 bg-amber-50 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">
                  Check for an existing client before creating a new Client ID.
                </p>
              </div>
            </div>
                ...
                .map((item) => (
                  <div key={item.id} className="p-4 grid gap-4 lg:grid-cols-[1fr_1.2fr_auto] lg:items-center">
                    <div>
                      ...
                      {item.status === "WAITING_DOCUMENTS" && (
                        <span className="mt-1 inline-block rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-bold text-rose-700">
                          Needs Correction
                        </span>
                      )}
                      ...
                      <div className="mt-2 rounded-lg bg-slate-50 p-2 text-xs text-slate-600">
                        <strong>
                          {item.policies?.length || 0} attached{" "}
                          {item.policies?.length === 1 ? "policy" : "policies"}
                        </strong>
                    ...
                    <div>
                      ...
                      {item.status === "WAITING_DOCUMENTS" ? (
                        <p className="rounded-lg bg-slate-100 px-3 py-2 text-xs text-slate-600">
                          Waiting for the requesting agent to correct and resubmit this Request ID.
                        </p>
                    ...
                    <div className="flex lg:flex-col gap-2">
                      ...
                      {item.status !== "WAITING_DOCUMENTS" && (
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800 hover:bg-amber-100"
                        >
                          Needs Correction
                        </button>
                      )}
                      {item.status !== "WAITING_DOCUMENTS" && (
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100"
                        >
                          Reject
                        </button>
                      )}
          ...
          <div className="flex items-center justify-between gap-4 lg:justify-end">
            ...
            {totalPages > 1 && (
              <div className="flex items-center rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
                ...
                <span className="min-w-16 px-2 text-center text-xs font-bold text-slate-700">
                  {page} / {totalPages}
                </span>
                <button
                  type="button"
        ...
        {loading ? (
          ...
          <div className="grid gap-3 bg-slate-50/50 p-4 md:p-5">
            {profiles.map((profile) => (
              <article
                key={profile.id}
                className="group grid gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_8px_24px_-22px_rgba(15,23,42,0.8)] transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_18px_32px_-24px_rgba(5,150,105,0.45)] lg:grid-cols-[1.25fr_1fr_auto] lg:items-center"
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex max-w-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                    <span
                      className="truncate font-mono text-[11px] font-medium text-slate-600"
                      title={profile.id}
                    >
                      {profile.id}
                    </span>
                    <button
                      ...
                      className="ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white text-slate-400 shadow-sm transition hover:text-emerald-600"
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {canResetMpin && (
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs font-bold text-amber-800 shadow-sm transition hover:bg-amber-100"
                    >
                      <KeyRound className="h-3.5 w-3.5" /> Reset MPIN
                    </button>
                  )}
                  <button
                    type="button"
                    onClick={() => handleOpenEditModal(profile)}
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 shadow-sm transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-800"
                  >
          ...
          >
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                onClick={() => setPage((value) => Math.max(value - 1, 1))}
                disabled={page === 1}
                className="inline-flex h-9 items-center gap-1 rounded-lg border border-slate-200 px-3 text-xs font-bold text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-35"
              >
              ...
              {paginationPages.map((pageNumber) => (
                <button
                  ...
                  className={`flex h-9 min-w-9 items-center justify-center rounded-lg px-2 text-xs font-bold transition ${pageNumber === page ? "bg-emerald-600 text-white shadow-md shadow-emerald-200" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}
                >
                  {pageNumber}
                </button>
              ))}
      ...
      {isModalOpen && (
        <ModalPortal>
          <div className="client-management-modal-shell">
            ...
            <div
              className="client-management-modal-card bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 animate-in fade-in zoom-in-95 duration-200"
              role="dialog"
              aria-modal="true"
            >
      ...
      {resolutionRequest && (
        <ModalPortal>
          <div className="client-management-modal-shell">
            ...
            />
            <div
              className="client-management-modal-card rounded-2xl border border-slate-100 bg-white shadow-2xl"
              role="dialog"
              aria-modal="true"
            >
                ...
                {resolutionAction === "LINK_EXISTING" ? (
                  ...
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-slate-700">
                    ...
                    <dl className="mt-3 grid grid-cols-[90px_1fr] gap-2 text-xs">
                      ...
                      <dd className="font-semibold">{resolutionRequest.name}</dd>
                      <dt className="text-slate-500">Phone</dt>
                      <dd className="font-semibold">{resolutionRequest.phone}</dd>
                      <dt className="text-slate-500">Email</dt>
                      <dd className="font-semibold">{resolutionRequest.email || "Not provided"}</dd>
                      <dt className="text-slate-500">Policies</dt>
                      <dd className="font-semibold">{resolutionRequest.policies?.length || 0}</dd>
                    </dl>
                    <p className="mt-3 text-xs text-amber-800">
                      Confirm only after checking that no valid existing client is available.
                    </p>
                  </div>
                )}
                {resolutionError && (
      ...
      {decisionRequest && (
        <ModalPortal>
          <div className="client-management-modal-shell">
            ...
            />
            <div
              className="client-management-modal-card rounded-2xl border border-white/70 bg-white shadow-2xl"
              role="dialog"
              aria-modal="true"
            >
              <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div>
                  <h3 className="font-bold text-slate-900">
                    {decisionAction === "REJECT" ? "Reject request for correction" : "Request correction"}
                  </h3>
                  <p className="mt-1 text-xs text-slate-500">
                    The same Request ID and attached policies will be preserved.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-4 p-6">
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Explain exactly what the agent must correct"
                    className="w-full resize-none rounded-xl border border-slate-300 px-3.5 py-3 text-sm focus:border-amber-500 focus:outline-none"
                  />
                </div>
                {resolutionError && (
        ...
        <ModalPortal>
          <div className="client-management-modal-shell">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                <div>
                  <h3 className="font-bold text-slate-900">Correct Client ID request</h3>
              ...
              <div className="space-y-4 p-6">
                {correctionRequest.correctionNote && (
                  <div className="rounded-xl border border-rose-100 bg-rose-50 p-3 text-sm text-rose-700">
                    <strong>Super Admin note:</strong> {correctionRequest.correctionNote}
                  </div>
                )}
                <div>
                  <label className="mb-1 block text-xs font-bold text-slate-600">CLIENT NAME</label>
                  <input
                    value={correctionName}
                    onChange={(event) => setCorrectionName(event.target.value)}
                    className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:outline-none"
                  />
                </div>
                <div>
