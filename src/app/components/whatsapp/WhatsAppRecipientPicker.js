              ...
              <div className="flex items-start justify-between gap-3 p-3">
                <div className="min-w-0">
                  <p className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-emerald-700">
                    <CheckCircle2 size={14} /> {matches.length === 1 ? "Auto matched" : `${matches.length} matching groups`}
                  </p>
                  {matches.slice(0, 5).map((group, index) => (
                    <label key={group.id} className="mt-2 flex cursor-pointer items-start gap-2 text-xs text-slate-800">
                      <input
                        type="radio"
                        name="matched-whatsapp-group"
                        checked={groupId === group.id}
                        disabled={disabled}
                        onChange={() => onGroupChange(group.id)}
                        className="mt-0.5 h-3.5 w-3.5 border-slate-300 text-emerald-600 focus:ring-emerald-500"
                      />
                      <span className={index === 0 ? "font-bold" : "font-medium"}>{group.name}</span>
                    </label>
                  ))}
                ...
                <button
                  ...
                  className="shrink-0 rounded-md border border-emerald-200 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-100"
                >
                  Change
                </button>
              </div>
