...
function looksTabular(line = "") {
  ...
  const labelHits = TABLE_TYPES.flatMap(([, patterns]) => patterns).filter((pattern) =>
    pattern.test(trimmed),
  ).length;
  const wideSpacing = /\S\s{2,}\S/.test(line);
  const denseLabelRun = /Registration.*Engine|Year.*Cubic.*Seating|Premium.*Tax|Own Damage.*Liability/i.test(
    trimmed,
  );
