function buildFieldMap(understanding = {}, schema = {}) {
  ...
  return fields.map((field) => {
    const section =
      sections.find((candidate) => candidate.type === field.section) ||
      findSectionByAliases(sections, field.aliases || []);
    const relatedTables = tables.filter(
      (table) => section && Math.abs(table.startLine - section.startLine) < 80,
    );
    return {
      field: field.field,
      ...
      relatedTables: relatedTables.map((table) => ({
        type: table.type,
        page: table.page,
...
function findSectionByAliases(sections, aliases) {
  return sections.find((section) =>
    aliases.some((alias) =>
      new RegExp(escapeRegExp(alias), "i").test(section.text || section.matchedLabel || ""),
    ),
  );
}
