...
import {
  DEFAULT_BANK_SOURCES,
  DEFAULT_CATEGORIES,
  DEFAULT_COMPANIES,
  DEFAULT_POLICY_TYPES,
  FIELD_TYPES,
} from "@/lib/policies/defaults";
  ...
  const [bankSources, companies, categories, policyTypes] = await Promise.all([
    ...
    prisma.policyType.findMany({
      ...
      include: {
        ...
        schemas: { where: { active: true }, include: { fields: { orderBy: { order: "asc" } } } },
      },
      orderBy: { name: "asc" },
    }),
  ]);
...
export async function createFieldSetup(payload) {
  ...
  const preparedPayload = {
    ...payload,
    categoryName: clean(payload.categoryName || payload.categoryId)
  ...
  const bankSource = preparedPayload.bankSourceName
    ? await prisma.bankSource.upsert({
        ...
        create: {
          name: clean(preparedPayload.bankSourceName),
          aliases: toList(preparedPayload.bankSourceAliases),
        },
      })
