...
function validatePolicyFields(data = {}) {
  ...
  addIssue(
    issues,
    validateIdentifierField("engineNumber", data.engineNumber, data.registrationNumber || data.vehicleNumber),
  );
...
function validatePolicyCrossFields(data = {}) {
  ...
  if (startDate && expiryDate && startDate >= expiryDate) {
    issues.push(
      issue("date_chronology", "Policy start date should be before expiry date.", [
        "startDate",
        "expiryDate",
      ]),
    );
  ...
  if (registration && engine && registration === engine) {
    issues.push(
      issue("registration_equals_engine", "Registration number cannot equal engine number.", [
        "registrationNumber",
        "engineNumber",
      ]),
    );
  ...
  if (chassis && engine && chassis === engine) {
    issues.push(
      issue("chassis_equals_engine", "Chassis number cannot equal engine number.", [
        "chassisNumber",
        "engineNumber",
      ]),
    );
  ...
  if (
    ...
    gstAmount != null &&
    totalPremium != null &&
    Math.abs(netPremium + gstAmount - totalPremium) > 2
  ) {
    issues.push(
      issue("premium_total_mismatch", "Net premium plus GST should approximately match total premium.", [
        "netPremium",
        "gstAmount",
        "totalPremium",
      ]),
  ...
  if (/\btwo\s*wheeler\b/i.test(data.policyType || "") && Number(data.seatingCapacity) >= 5) {
    issues.push(
      issue("two_wheeler_seating", "Two wheeler policy has an unusually high seating capacity.", [
        "policyType",
        "seatingCapacity",
      ]),
    );
...
function validateRegistrationNumber(value) {
  ...
  if (!REGISTRATION_PATTERN.test(clean)) {
    return issue(
      "registration_format",
      "Registration number does not match a standard Indian vehicle format.",
      ["registrationNumber"],
    );
  }
  return null;
...
function validateIdentifierField(field, value, registrationNumber) {
  ...
  if (cleanRegistration(registrationNumber) === clean) {
    return issue(`${field}_matches_registration`, `${field} cannot match the registration number.`, [
      field,
      "registrationNumber",
    ]);
  }
...
function validateChassisNumber(value, engineNumber) {
  ...
  if (clean.length < 8 || /^(?:CHASSIS|CHASSISNO|ENGINE|ENGINENO)$/i.test(clean)) {
    return issue("chassis_format", "Chassis number should be a realistic VIN/chassis identifier.", [
      "chassisNumber",
    ]);
  }
  if (cleanIdentifier(engineNumber) === clean) {
    return issue("chassis_matches_engine", "Chassis number cannot match engine number.", [
      "chassisNumber",
      "engineNumber",
    ]);
  }
...
function validatePolicyNumber(value) {
  ...
  if (clean.length < 5 || !/\d/.test(clean)) {
    return issue(
      ...
      "Policy number should contain a stable alphanumeric policy identifier.",
      ["policyNumber"],
    );
  }
  return null;
