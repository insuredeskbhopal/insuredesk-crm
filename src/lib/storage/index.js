...
const SIGNING_SECRET =
  process.env.JWT_SECRET || (process.env.NODE_ENV === "production" ? "" : "dev-only-storage-signing-secret");
if (!SIGNING_SECRET) {
...
export async function uploadFile(buffer, mimeType, filename) {
  ...
  if (STORAGE_PROVIDER === "s3") {
    ...
    if (!BUCKET || !process.env.AWS_ACCESS_KEY_ID) {
      throw new Error(
        "S3 storage is missing required credentials. Uploaded PDFs must remain in cloud storage.",
      );
    }
