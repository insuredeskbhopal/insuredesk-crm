...
function parseRows() {
  ...
  for (const line of lines) {
    ...
    // Find fields dynamically based on patterns
    // Date is dd-mm-yyyy
    const sno = parts[0];
    const date = parts[1];
    const insuredName = parts[2];
    
    // Find vehicle number: typically starts with MP, UP, RJ, DL, HR, MH, PB, GA, or 'NEW'
    let veh = '';
    let polNum = '';
    let contactName = '';
    let mobile = '';
    ...
    for (let i = 3; i < parts.length; i++) {
      const p = parts[i];
      if (!p) continue;
      if (/^[A-Z]{2}\s*\d{1,2}\s*[A-Z]{1,3}\s*\d{1,4}$/i.test(p) || (p === 'NEW' && !veh)) {
        veh = p;
      } else if (/^\d{10}$|^\d{5}\s*\d{5}$|^\d{2}\s*\d{4}\s*\d{4}$|^\d{10}\/\s*\d{10}$/i.test(p.replace(/["\r\n]/g, '').trim())) {
        mobile = p.replace(/["\r\n]/g, '').trim();
      }
    }
...
async function main() {
  const augMotorDir = path.resolve(__dirname, '../storage/aug motor');
  const storageDir = path.resolve(__dirname, '../storage');

  const augFiles = fs.readdirSync(augMotorDir).filter(f => f.toLowerCase().endsWith('.pdf'));
  console.log(`Found ${augFiles.length} files in storage/aug motor`);

  const storageFiles = fs.readdirSync(storageDir).filter(f => f.toLowerCase().endsWith('.pdf'));
  console.log(`Found ${storageFiles.length} files in storage/`);

  // Parse table rows carefully
  const rows = parseRows();
  console.log(`Parsed ${rows.length} rows from table.`);
  
  // Also collect all PDFs across both dirs and inspect their text / vehicle numbers / policy numbers
  const allPdfs = [];
  for (const f of augFiles) {
    allPdfs.push({ filename: f, dir: augMotorDir });
  }
  for (const f of storageFiles) {
    allPdfs.push({ filename: f, dir: storageDir });
  }

  const pdfDetails = [];
  for (const p of allPdfs) {
    try {
      const buf = fs.readFileSync(path.join(p.dir, p.filename));
      const pdata = await pdf(buf);
      const text = pdata.text;
      const extracted = extractPolicyFromText(text, p.filename);
      pdfDetails.push({
        filename: p.filename,
        dir: p.dir,
        regNo: extracted.registrationNumber || '',
        polNo: extracted.policyNumber || '',
        insuredName: extracted.insuredName || '',
        company: extracted.insuranceCompany || '',
        gross: extracted.totalPremium || '',
        net: extracted.netPremium || '',
        extracted
      });
    } catch (e) {
      console.log(`Error parsing PDF ${p.filename}:`, e.message);
    }
  }

  console.log(`Extracted metadata from ${pdfDetails.length} PDFs.`);
  fs.writeFileSync('scripts/pdf_details_debug.json', JSON.stringify(pdfDetails, null, 2));
}
