...
// Check where each row's PDF is located
const pdfDetails = JSON.parse(fs.readFileSync('scripts/pdf_details_debug.json', 'utf8'));
const rows = parseRows();

// Special cases:
// Rows 8, 9, 10 -> VKMCPL_CHASSIS
// Row 3 -> SANJAY KUMAR SONI_MP37C5791
// Row 47 -> PRAGATI FOODS_TZ220XS17CO2605207_2027-26.pdf
// Row 46 -> Siddharth Nahar? Let's check where Siddharth Nahar PDF is
// Row 55 -> SUNIL MAHESHWARI MP004LD2492 -> SUNIL MAHESHWARI_MP04LD2492_2026-27 POLICY.pdf
...
for (const r of rows) {
  ...
  if (r.insuredName.includes('VIJAY KUMAR MISHRA PVT') && r.vehNo === 'NEW') {
    const chassis = pdfDetails.find(p => p.filename.includes('VKMCPL_CHASSIS') && !usedPdfs.has(p.filename));
    if (chassis) matched = chassis;
  } else if (r.sno === '3' && r.insuredName.includes('SANJAY KUMAR SONI')) {
    matched = pdfDetails.find(p => p.filename.includes('SANJAY KUMAR SONI_MP37C5791'));
  } else if (r.insuredName.includes('PRAGATI FOODS')) {
    matched = pdfDetails.find(p => p.filename.includes('PRAGATI FOODS'));
  } else if (cleanVeh && cleanVeh !== 'NEW') {
    matched = pdfDetails.find(p => {
  ...
  if (!matched) {
    matched = pdfDetails.find(p => {
      const pName = normalize(p.insuredName);
      const pFile = normalize(p.filename);
      return cleanName.length > 6 && (pName.includes(cleanName) || pFile.includes(cleanName));
    });
  }

  if (matched) {
    usedPdfs.add(matched.filename);
    results.push({ row: r, pdf: matched, status: 'FOUND' });
  } else {
    results.push({ row: r, pdf: null, status: 'NOT_FOUND' });
  }
}

console.log(`Matched PDFs: ${results.filter(r => r.status === 'FOUND').length} / ${rows.length}`);
const notFound = results.filter(r => r.status === 'NOT_FOUND');
console.log(`PDF Not Found count: ${notFound.length}`);
for (const nf of notFound) {
  console.log(`Sno: ${nf.row.sno} | Name: ${nf.row.insuredName} | Veh: ${nf.row.vehNo} | Pol: ${nf.row.polNo}`);
}
