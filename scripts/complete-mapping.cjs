...
const localPdfs = JSON.parse(fs.readFileSync('scripts/pdf_details_debug.json', 'utf8'));
const driveFiles = JSON.parse(fs.readFileSync('scripts/drive_files.json', 'utf8'));
const dbData = JSON.parse(fs.readFileSync('scripts/db_light_analysis.json', 'utf8'));

const finalMapping = [];
const usedLocal = new Set();
const usedDrive = new Set();

for (const r of rows) {
  const cleanVeh = normalize(r.vehNo).replace('MP004', 'MP04');
  const cleanPol = normalize(r.polNo);
  const cleanName = normalize(r.insuredName);
  ...
  if (r.insuredName.includes('VIJAY KUMAR MISHRA PVT') && r.vehNo === 'NEW') {
    const ch = localPdfs.find(p => p.filename.includes('VKMCPL_CHASSIS') && !usedLocal.has(p.filename));
    if (ch) localMatch = ch;
  } else if (r.sno === '3' && r.insuredName.includes('SANJAY KUMAR SONI')) {
    localMatch = localPdfs.find(p => p.filename.includes('SANJAY KUMAR SONI_MP37C5791'));
  } else if (r.insuredName.includes('PRAGATI FOODS')) {
    localMatch = localPdfs.find(p => p.filename.includes('PRAGATI FOODS'));
  } else if (cleanVeh && cleanVeh !== 'NEW') {
    localMatch = localPdfs.find(p => {
      const pReg = normalize(p.regNo);
      const pFile = normalize(p.filename);
      return pReg === cleanVeh || pFile.includes(cleanVeh);
    });
  }

  if (!localMatch && cleanPol) {
    localMatch = localPdfs.find(p => {
      const pPol = normalize(p.polNo);
      return pPol && (pPol === cleanPol || (cleanPol.length > 6 && pPol.includes(cleanPol)));
    });
  }

  if (localMatch) {
    usedLocal.add(localMatch.filename);
  }

  // 2. Check Drive match
  if (cleanVeh && cleanVeh !== 'NEW') {
    driveMatch = driveFiles.find(df => normalize(df.name).includes(cleanVeh));
  }
  if (!driveMatch && cleanPol && cleanPol.length > 6) {
    driveMatch = driveFiles.find(df => normalize(df.name).includes(cleanPol));
  }
  if (!driveMatch && cleanVeh === 'NEW' && cleanName.length > 6) {
    driveMatch = driveFiles.find(df => normalize(df.name).includes(cleanName));
  }

  // 3. Check DB match
  const dbMatch = dbData.find(d => {
    const dr = d.row;
    return dr.sno === r.sno && dr.insuredName === r.insuredName && dr.vehNo === r.vehNo;
  });
  ...
  finalMapping.push({
    ...
    net: r.net,
    company: r.company,
    inDb: dbMatch && dbMatch.status === 'EXISTS',
    dbRecordId: dbMatch?.matches?.[0]?.id || null,
    dbStoragePath: dbMatch?.matches?.[0]?.storagePath || null,
    dbStorageProvider: dbMatch?.matches?.[0]?.storageProvider || null,
    localPdf: localMatch ? { filename: localMatch.filename, dir: localMatch.dir } : null,
    drivePdf: driveMatch ? { name: driveMatch.name, id: driveMatch.id } : null
  });
}

const summary = {
  total: rows.length,
  hasLocalPdf: finalMapping.filter(m => m.localPdf).length,
  hasDrivePdf: finalMapping.filter(m => m.drivePdf).length,
  hasAnyPdf: finalMapping.filter(m => m.localPdf || m.drivePdf || m.dbStoragePath).length,
  inDbCount: finalMapping.filter(m => m.inDb).length,
  missingPdfCount: finalMapping.filter(m => !m.localPdf && !m.drivePdf && !m.dbStoragePath).length
};

console.log('Summary:', summary);
const missing = finalMapping.filter(m => !m.localPdf && !m.drivePdf && !m.dbStoragePath);
if (missing.length > 0) {
  console.log('Missing all sources count:', missing.length);
  for (const m of missing) {
    console.log(`Sno: ${m.sno} | ${m.name} | Veh: ${m.veh} | Pol: ${m.pol}`);
  }
}

fs.writeFileSync('scripts/complete_100_mapping.json', JSON.stringify(finalMapping, null, 2));
