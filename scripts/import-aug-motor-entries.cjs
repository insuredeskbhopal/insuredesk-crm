...
const { uploadGoogleDriveFile, downloadGoogleDriveFile } = require('../src/lib/storage/google-drive-storage.js');
...
async function main() {
  console.log('=== STARTING AUGUST MOTOR POLICY ENTRY & PDF ENRICHMENT ===');
  const uploadDir = await ensureLocalUploadDir();
  const rows = parseRows();
  console.log(`Total rows to process: ${rows.length}`);

  // Load pre-audited complete mapping
  ...
  for (let i = 0; i < mapping.length; i++) {
    ...
    const targetDate = parseDateToAugust(item.date);
    const logPrefix = `[${i + 1}/${mapping.length}] (Sno: ${item.sno || '-'}) ${item.name} [${item.veh || item.pol}]`;

    try {
      // 1. Resolve PDF Buffer & Info if available
      let pdfBuffer = null;
      let pdfFilename = item.localPdf?.filename || item.drivePdf?.name || null;
      let driveFileId = item.dbStoragePath || item.drivePdf?.id || null;

      // Try local read first
      if (item.localPdf) {
        const localPath = path.join(item.localPdf.dir, item.localPdf.filename);
        if (fs.existsSync(localPath)) {
          pdfBuffer = fs.readFileSync(localPath);
          pdfFilename = item.localPdf.filename;
        }
      }

      // If no local buffer but we have a driveFileId, download from Google Drive
      if (!pdfBuffer && driveFileId) {
        try {
          pdfBuffer = await downloadGoogleDriveFile(driveFileId);
          console.log(`  ${logPrefix}: Downloaded from Google Drive (${pdfBuffer.length} bytes)`);
        } catch (err) {
          console.warn(`  ${logPrefix}: Warning downloading from Drive (${driveFileId}):`, err.message);
        }
      }

      // If we have buffer but no driveFileId, upload to Google Drive so it's downloadable from anywhere
      if (pdfBuffer && !driveFileId) {
        try {
          console.log(`  ${logPrefix}: Uploading to Google Drive...`);
          const fileHash = calculateHash(pdfBuffer);
          const uploaded = await uploadGoogleDriveFile({
            buffer: pdfBuffer,
            mimeType: 'application/pdf',
            filename: pdfFilename || 'policy.pdf',
            fileHash,
            fileSize: pdfBuffer.length
          });
          driveFileId = uploaded.storagePath;
          console.log(`  ${logPrefix}: Uploaded to Drive -> ID: ${driveFileId}`);
        } catch (err) {
          console.warn(`  ${logPrefix}: Drive upload failed, will fallback to local storage:`, err.message);
        }
      }

      // Also save local copy in storage/uploads/2026/08/
      let localRelativeStoragePath = null;
      if (pdfBuffer && pdfFilename) {
        const localDest = path.join(uploadDir, pdfFilename);
        fs.writeFileSync(localDest, pdfBuffer);
        localRelativeStoragePath = `2026/08/${pdfFilename}`.replace(/\\/g, '/');
      }

      // Extract PDF data if we have buffer
      let extractedPdfData = null;
      if (pdfBuffer) {
        try {
          const parsed = await pdf(pdfBuffer);
          extractedPdfData = extractPolicyFromText(parsed.text, pdfFilename || 'policy.pdf');
        } catch (err) {
          console.warn(`  ${logPrefix}: Warning extracting text from PDF:`, err.message);
        }
      }

      // 2. Process EXISTING Record vs NEW Record
      if (item.inDb && item.dbRecordId) {
        // Fetch current record
        const currentRecord = await prisma.policyRecord.findUnique({
          where: { id: item.dbRecordId },
          include: { uploadedFile: true }
        });

        if (!currentRecord) {
          console.warn(`  ${logPrefix}: Record ${item.dbRecordId} not found in DB!`);
          continue;
        }

        // Prepare updated data & reviewedData
        // Priority: Extracted PDF data as base (if available) -> overlay existing -> overlay user table contact & August dates
        const baseData = extractedPdfData || currentRecord.reviewedData || currentRecord.data || {};
        ...
        const mergedReviewedData = {
          ...
          policyStartDate: targetDate.toISOString().slice(0, 10),
          startDate: targetDate.toISOString().slice(0, 10),
          contactPerson: item.contactName || baseData.contactPerson || '',
          contactPersonName: item.contactName || baseData.contactPersonName || '',
          contactNumber: item.mobile || baseData.contactNumber || '',
          customerMobile: item.mobile || baseData.customerMobile || '',
          insuredName: extractedPdfData?.insuredName || baseData.insuredName || item.name,
          registrationNumber: extractedPdfData?.registrationNumber || baseData.registrationNumber || (item.veh !== 'NEW' ? item.veh : ''),
          totalPremium: extractedPdfData?.totalPremium || baseData.totalPremium || item.gross,
          netPremium: extractedPdfData?.netPremium || baseData.netPremium || item.net,
          insuranceCompany: extractedPdfData?.insuranceCompany || baseData.insuranceCompany || item.company,
          policyType: extractedPdfData?.policyType || baseData.policyType || item.polType
        };
        ...
        // Manage UploadedFile
        let uploadedFileId = currentRecord.uploadedFileId;

        if (pdfBuffer) {
          const fileHash = calculateHash(pdfBuffer);
          const fileSize = pdfBuffer.length;
          const storageProvider = driveFileId ? 'google_drive' : 'local';
          const storagePath = driveFileId || localRelativeStoragePath;

          if (uploadedFileId) {
            await prisma.uploadedFile.update({
              where: { id: uploadedFileId },
              data: {
                sourceFile: pdfFilename || currentRecord.pdfFileName || 'policy.pdf',
                storageProvider,
                storagePath,
                fileSize,
                sizeBytes: fileSize,
            ...
            const newUpload = await prisma.uploadedFile.create({
              data: {
                ...
                sourceFile: pdfFilename || 'policy.pdf',
                storageProvider,
                storagePath,
                fileSize,
                sizeBytes: fileSize,
        ...
        await prisma.policyRecord.update({
          ...
          data: {
            ...
            contactPersonName: item.contactName || null,
            contactPersonMobile: item.mobile || null,
            renewalRecipientName: item.contactName || item.name,
            renewalRecipientMobile: item.mobile || null,
            uploadedFileId: uploadedFileId || currentRecord.uploadedFileId,
            pdfFileName: pdfFilename || currentRecord.pdfFileName,
            pdfBytes: pdfBuffer || currentRecord.pdfBytes,
        ...
        updatedCount++;
        console.log(`✓ UPDATED: ${logPrefix} -> Date: ${targetDate.toISOString().slice(0, 10)}, Contact: ${item.contactName} (${item.mobile}), PDF: ${pdfFilename || 'None'}`);

      } else {
        // CREATE NEW PolicyRecord
        ...
        const fullPayload = {
          ...(extractedPdfData || {}),
          insuredName: extractedPdfData?.insuredName || item.name,
          registrationNumber: extractedPdfData?.registrationNumber || (item.veh !== 'NEW' ? item.veh : ''),
          policyNumber: extractedPdfData?.policyNumber || item.pol || '',
          totalPremium: extractedPdfData?.totalPremium || item.gross,
          netPremium: extractedPdfData?.netPremium || item.net,
          insuranceCompany: extractedPdfData?.insuranceCompany || item.company,
          policyType: extractedPdfData?.policyType || item.polType,
        ...
        const newRec = await prisma.policyRecord.create({
          data: {
            ...
            pdfBytes: pdfBuffer,
            sourceFile: pdfFilename,
            detectedCompany: fullPayload.insuranceCompany,
            selectedCompany: fullPayload.insuranceCompany,
            detectedServiceCategory: 'Motor Insurance',
            selectedServiceCategory: 'Motor Insurance',
            detectedPolicyType: fullPayload.policyType,
            selectedPolicyType: fullPayload.policyType,
        ...
        createdCount++;
        console.log(`+ CREATED: ${logPrefix} -> ID: ${newRec.id}, Date: ${targetDate.toISOString().slice(0, 10)}, Contact: ${item.contactName} (${item.mobile}), PDF: ${pdfFilename || 'None'}`);
      }

    } catch (rowErr) {
      console.error(`✗ ERROR on ${logPrefix}:`, rowErr);
      skippedCount++;
    }
  }

  console.log('\n=== EXECUTION FINISHED ===');
  console.log(`Total rows processed: ${mapping.length}`);
  console.log(`Records Updated: ${updatedCount}`);
  console.log(`Records Created: ${createdCount}`);
  console.log(`PDFs Attached/Created: ${pdfAttachedCount}`);
