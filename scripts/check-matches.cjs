...
// Let's inspect rows that have duplicate PDF matches or special vehicle numbers
// Specifically:
// 1. VKMCPL chassis files vs rows 8, 9, 10
// 2. Row 3 (SANJAY KUMAR SONI MP04CA2453 in table, but in PDF it's MP37C5791!)
// 3. Row 46 (Mr Siddharth Nahar NEW)
// 4. Row 47 (PRAGATI FOODS NEW)
// 5. SUNIL MAHESHWARI MP004LD2492 vs MP04LD2492

// Let's find matches for all rows carefully
const usedPdfs = new Set();
const rowMatches = [];

for (const r of rows) {
  let matchedPdf = null;
  const normV = normalizeVeh(r.vehNo);

  // Special cases:
  // Vijay Kumar Mishra PVT rows 8, 9, 10 (Chassis 07214, 07341, 07342)
  if (r.insuredName.includes('VIJAY KUMAR MISHRA PVT') && r.vehNo === 'NEW') {
    // Find an unused VKMCPL_CHASSIS file
    const chassisPdfs = pdfDetails.filter(p => p.filename.includes('VKMCPL_CHASSIS') && !usedPdfs.has(p.filename));
    if (chassisPdfs.length > 0) {
      matchedPdf = chassisPdfs[0];
    }
  } else if (r.sno === '3' && r.insuredName.includes('SANJAY KUMAR SONI')) {
    // PDF is SANJAY KUMAR SONI_MP37C5791_2026-27 POLICY.pdf
    matchedPdf = pdfDetails.find(p => p.filename.includes('SANJAY KUMAR SONI_MP37C5791'));
  } else if (r.insuredName.includes('Siddharth Nahar') && r.vehNo === 'NEW') {
    // Let's find Siddharth Nahar PDF
    matchedPdf = pdfDetails.find(p => p.filename.toLowerCase().includes('nahar') || p.insuredName.toLowerCase().includes('nahar'));
  } else if (r.insuredName.includes('PRAGATI FOODS') && r.vehNo === 'NEW') {
    matchedPdf = pdfDetails.find(p => p.filename.includes('PRAGATI FOODS'));
  } else if (normV && normV !== 'NEW') {
    // Search by normV
    // Handle MP004LD2492 -> MP04LD2492
    const cleanV = normV.replace('MP004', 'MP04');
    matchedPdf = pdfDetails.find(p => {
      const pReg = normalizeVeh(p.regNo);
      const pFile = normalizeVeh(p.filename);
      return pReg === cleanV || pReg === normV || pFile.includes(cleanV) || pFile.includes(normV);
    });
  }

  // Fallback to polNo
  if (!matchedPdf && r.polNo) {
    const cleanP = r.polNo.replace(/[^A-Za-z0-9]/g, '').toUpperCase();
    matchedPdf = pdfDetails.find(p => {
      const pPol = (p.polNo || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase();
      return pPol && (pPol.includes(cleanP) || cleanP.includes(pPol));
    });
  }

  if (matchedPdf) {
    usedPdfs.add(matchedPdf.filename);
    rowMatches.push({ row: r, pdf: matchedPdf, status: 'MATCHED' });
  } else {
    rowMatches.push({ row: r, pdf: null, status: 'UNMATCHED' });
  }
}

const unmatched = rowMatches.filter(m => m.status === 'UNMATCHED');
console.log(`Unmatched rows count: ${unmatched.length}`);
if (unmatched.length > 0) {
  console.log('Unmatched rows:', JSON.stringify(unmatched.map(u => u.row), null, 2));
}

// Check for any duplicate PDF usage
const pdfUsageCount = {};
for (const m of rowMatches) {
  if (m.pdf) {
    pdfUsageCount[m.pdf.filename] = (pdfUsageCount[m.pdf.filename] || 0) + 1;
  }
}
const duplicatePdfs = Object.entries(pdfUsageCount).filter(([fn, count]) => count > 1);
console.log(`Duplicate PDF usage count: ${duplicatePdfs.length}`);
if (duplicatePdfs.length > 0) {
  console.log('Duplicates:', duplicatePdfs);
}

// Check which PDFs in aug motor are unused
const augMotorFiles = pdfDetails.filter(p => p.dir.includes('aug motor'));
const unusedAugPdfs = augMotorFiles.filter(p => !usedPdfs.has(p.filename));
console.log(`Unused PDFs in aug motor (${unusedAugPdfs.length}):`);
for (const p of unusedAugPdfs) {
