...
async function main() {
  ...
  console.log(`Querying DB lightweight index...`);
  const allRecords = await prisma.$queryRaw`
    SELECT 
      pr.id,
      pr.saved_at,
      pr.created_at,
      pr.contact_person_name,
      pr.contact_person_mobile,
      pr.pdf_file_name,
      pr.source_file,
      pr.reviewed_data->>'registrationNumber' as reg_no,
      pr.reviewed_data->>'vehicleNumber' as veh_no,
      pr.reviewed_data->>'policyNumber' as pol_no,
      pr.reviewed_data->>'insuredName' as insured_name,
      pr.reviewed_data->>'startDate' as start_date,
      pr.data->>'registrationNumber' as d_reg_no,
      pr.data->>'policyNumber' as d_pol_no,
      pr.data->>'insuredName' as d_insured_name,
  ...
  `;
  console.log(`Loaded ${allRecords.length} records in lightweight mode.`);
  ...
  for (const r of rows) {
    ...
    // Search in loaded records
    const matched = allRecords.filter(rec => {
      const recVeh = normalize(rec.reg_no || rec.veh_no || rec.d_reg_no);
      const recPol = normalize(rec.pol_no || rec.d_pol_no);
      const recName = normalize(rec.insured_name || rec.d_insured_name);
      const recFile = normalize(rec.pdf_file_name || rec.source_file);

      if (cleanVeh && cleanVeh !== 'NEW' && (recVeh === cleanVeh || recFile.includes(cleanVeh))) {
        return true;
      }
      if (cleanPol && (recPol === cleanPol || (cleanPol.length > 6 && recPol.includes(cleanPol)))) {
        return true;
      }
      if (cleanVeh === 'NEW' && cleanName.length > 5 && (recName.includes(cleanName) || cleanName.includes(recName))) {
        return true;
      }
      return false;
    });
    ...
    if (matched.length > 0) {
      ...
      analysis.push({
        ...
        matches: matched.map(m => ({
          ...
          contactPersonName: m.contact_person_name,
          contactPersonMobile: m.contact_person_mobile,
          pdfFileName: m.pdf_file_name,
          uploadedFileId: m.uploaded_file_id,
  ...
  console.log(`Found in DB: ${foundCount} / ${rows.length}`);
  console.log(`Not found in DB: ${notFoundCount} / ${rows.length}`);

  const notFoundList = analysis.filter(a => a.status === 'NOT_FOUND');
  console.log(`\n--- NOT FOUND IN DB (${notFoundList.length}) ---`);
  for (const nf of notFoundList) {
    console.log(`Sno: ${nf.row.sno} | Date: ${nf.row.date} | Name: ${nf.row.insuredName} | Veh: ${nf.row.vehNo} | Pol: ${nf.row.polNo}`);
  }

  fs.writeFileSync('scripts/db_light_analysis.json', JSON.stringify(analysis, null, 2));
}
