...
describe("renewal policy type display", () => {
  it("shows broad motor policy families instead of cover wording", () => {
    ...
    expect(normalizePolicyFamily({ policyType: "Goods Carrying Vehicle Policy - Liability only [Reprint]" })).toBe("Motor Policy");
    expect(normalizePolicyFamily({ policyType: "Zero Depreciation", vehicleNumber: "MP04AB1234" })).toBe("Motor Policy");
  });

  it("maps common non-motor policy types to families", () => {
    expect(normalizePolicyFamily({ policyType: "Standard Fire and Special Perils Policy" })).toBe("Fire Policy");
